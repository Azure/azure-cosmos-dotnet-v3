    ...
    internal sealed class KeyVaultAccessClient : IKeyVaultAccessClient
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            using (HttpResponseMessage response = await this.InternalGetKeyAsync(keyVaultKeyUri, accessToken, cancellationToken))
            {
                ...
                if (response.StatusCode != HttpStatusCode.OK)
                {
                    DefaultTrace.TraceInformation("GetKeyResponseAsync: Receive HttpStatusCode {0}, KeyVaultErrorCode {1}, Errors: {2}. ", response.StatusCode, KeyVaultErrorCode.InternalServerError, jsonResponse);
                    throw new KeyVaultAccessException(
                        response.StatusCode,
                        KeyVaultErrorCode.InternalServerError,
                        jsonResponse);
