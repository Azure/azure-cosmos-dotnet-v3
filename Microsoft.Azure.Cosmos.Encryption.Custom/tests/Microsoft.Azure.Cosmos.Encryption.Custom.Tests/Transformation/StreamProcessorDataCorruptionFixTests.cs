...
namespace Microsoft.Azure.Cosmos.Encryption.Tests.Transformation
{
    ...
    public class StreamProcessorDataCorruptionFixTests
    {
        ...
        [TestMethod]
        public async Task Decrypt_TamperedLongMarkerProducingInfinity_ThrowsForBothProcessors()
        {
            const long InfinityBitPattern = 9218868437227405312L;
            MemoryStream encrypted = await EncryptJsonAsync(
                $"{{\"id\":\"1\",\"n\":{InfinityBitPattern}}}",
                CreateOptions(new[] { "/n" }));
            encrypted.Position = 0;
            using StreamReader reader = new(encrypted, leaveOpen: true);
            JObject tampered = JObject.Parse(await reader.ReadToEndAsync());
            byte[] cipherTextWithTypeMarker = Convert.FromBase64String(tampered["n"].Value<string>());
            cipherTextWithTypeMarker[0] = (byte)TypeMarker.Double;
            tampered["n"] = Convert.ToBase64String(cipherTextWithTypeMarker);
            EncryptionProperties properties = tampered[Constants.EncryptedInfo].ToObject<EncryptionProperties>();

            using MemoryStream streamInput = new(
                Encoding.UTF8.GetBytes(tampered.ToString(Newtonsoft.Json.Formatting.None)));
            using MemoryStream streamOutput = new();
            await Assert.ThrowsExceptionAsync<InvalidOperationException>(
                () => new StreamProcessor().DecryptStreamAsync(
                    streamInput,
                    streamOutput,
            ...
            JObject newtonsoftDocument = (JObject)tampered.DeepClone();
            await Assert.ThrowsExceptionAsync<InvalidOperationException>(
