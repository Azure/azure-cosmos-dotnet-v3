...
#### Added
- [#PLACEHOLDER](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/PLACEHOLDER) Adds a typed, discoverable API for selecting the encryption JSON processor per operation on `net8.0`, replacing the magic-string `RequestOptions.Properties["encryption-json-processor"]` bag. `JsonProcessor` is now a public enum; `RequestOptions.WithEncryptionJsonProcessor(JsonProcessor)` selects the processor fluently on any feed request options (query, change-feed, read-many), and new `Container.ToEncryptionFeedIterator<T>(IQueryable<T>, JsonProcessor)` / `ToEncryptionStreamIterator<T>(IQueryable<T>, JsonProcessor)` overloads select it for LINQ. The property-bag key continues to work for back-compat. Default remains Newtonsoft; existing callers see no behavioral change.
