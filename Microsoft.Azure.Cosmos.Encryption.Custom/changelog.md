...
#### Fixes
- Restores compatibility for custom `Encryptor` and `DataEncryptionKey` subclasses written against `1.0.0-preview08`: the members added since (`Encryptor.GetEncryptionKeyAsync` and the buffer-based `DataEncryptionKey` members `EncryptData`/`DecryptData`/`GetEncryptByteCount`/`GetDecryptByteCount`) are now `virtual` with safe default implementations instead of `abstract`, so existing subclasses keep compiling and loading. Custom `Encryptor` implementations that do not override `GetEncryptionKeyAsync` are again dispatched through their `EncryptAsync`/`DecryptAsync` overrides on the MDE encryption path (these overrides were silently bypassed in interim builds); `DataEncryptionKey` implementations that only override the array-based `EncryptData`/`DecryptData` are routed through those members.
- Restores the stable `Microsoft.Azure.Cosmos` package dependency for non-preview builds (`IsPreview != true`); only preview builds reference the preview core SDK. Stable-pinned consumers no longer pull a preview Cosmos package transitively.
- Hardens the new opt-in Stream JSON processor (`net8.0`-only, request-options key `encryption-json-processor`; never released) against data corruption before its first release:
  - Pass-through strings and property names containing JSON escape sequences (quotes, backslashes, control characters, `\uXXXX`) are no longer double-escaped during encryption or decryption, including inside encrypted object/array payloads.
  - Property-name matching against `PathsToEncrypt`/`_ep` is now strictly top-level: nested properties sharing an encrypted path's name are no longer wrongly encrypted/decrypted, `_ep` bookkeeping can no longer record wrong or duplicate paths, and nested user properties named `_ei` are preserved.
  - A JSON `null` inside an encrypted object/array no longer corrupts the `_ep` metadata (previously rendered the document undecryptable on every SDK version).
  - Encrypting a document that already contains a top-level `_ei` property now fails with a clear error (parity with the Newtonsoft processor) instead of emitting duplicate `_ei` keys.
- Number-handling parity between the two JSON processors: integer literals outside the `Int64` range are rejected by the Stream processor (previously silently encrypted as a lossy `double`), matching the Newtonsoft processor. Non-finite doubles (`NaN`, `Infinity`, `-Infinity`, e.g. from the overflowing literal `1e309`) are cleanly supported by both processors: because they cannot be represented as a serialized SQL float or a JSON number, they are encrypted as their canonical string form (`"NaN"`/`"Infinity"`/`"-Infinity"`) and decrypt to that same string identically on both processors, matching the platform's default `FloatFormatHandling.String` representation and remaining readable after a downgrade to `1.0.0-preview08`. Decrypted integral doubles are written as `5.0` instead of `5` by the Stream processor so the value's type no longer flips between double and long across encrypt/decrypt cycles.
- Decrypting a legacy-algorithm (`AEAes256CbcHmacSha256Randomized`, format version 2) document through the Stream-processor decrypt API now produces an error that names the legacy format and points to the Newtonsoft processor, instead of a misleading "unknown format version" message.
- A document whose `_ei` property holds a non-object value (not a valid encryption envelope) is now passed through unchanged by the Stream processor, matching the Newtonsoft processor and the originally released behavior, instead of throwing.
