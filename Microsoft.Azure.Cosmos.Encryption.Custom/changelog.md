...
#### Fixes
- [#5986](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/5986) Stream-mode JSON processing now preserves escaped property names and string values, nested nulls, integral doubles, and large integer fidelity without corrupting encrypted documents. It also rejects out-of-range integer literals, non-finite values produced by tampered type markers, and pre-existing top-level encryption metadata; promptly clears temporary pooled plaintext buffers; accepts quoted encryption-format versions; disposes successfully-consumed input streams; supports Newtonsoft fallback for non-seekable feeds; and reports the processor actually used for feed decryption through the existing `ActivitySource`.
