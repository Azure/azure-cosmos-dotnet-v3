...
- Number-handling parity between the two JSON processors: integer literals outside the `Int64` range are now rejected by the Stream processor (previously silently encrypted as a lossy `double`), and non-finite doubles (e.g. the literal `1e309` parsing to Infinity) are now rejected by the Newtonsoft processor (previously silently encrypted, decrypting to a string). Decrypted integral doubles are written as `5.0` instead of `5` by the Stream processor so the value's type no longer flips between double and long across encrypt/decrypt cycles.
