...
#### Fixes
- [#6041](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/6041) Fixes cross-provider cache interference in processes that construct more than one `CosmosDataEncryptionKeyProvider` (for example multi-tenant hosts). Each provider's `EncryptionKeyStoreProvider.DataEncryptionKeyCacheTimeToLive` now applies only to that provider, instead of the most recently constructed provider's value governing every provider in the process. As part of this fix, setting `DataEncryptionKeyCacheTimeToLive` to `TimeSpan.Zero` reliably keeps decrypted key material from being cached across operations. Requires `Microsoft.Data.Encryption.Cryptography` `2.0.0-pre024`.
