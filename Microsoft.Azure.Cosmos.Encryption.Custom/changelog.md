...
#### Fixes
- [#6105](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/6105) Fixes cancellation handling for MDE operations using custom `Encryptor` implementations: pre-canceled operations are rejected when configured properties are missing or null, and cancellation stops waiting for asynchronous encryptor calls that ignore the token.
- [#5428](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/5428) Fixes stale unwrapped-key cache entries after key properties are refreshed or removed by invalidating entries under the DEK ID and self link.
