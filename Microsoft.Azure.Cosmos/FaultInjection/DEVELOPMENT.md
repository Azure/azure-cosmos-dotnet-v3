...
| `ArgumentOutOfRangeException` on `WithInjectionRate` / `WithThreshold` / `SetInjectionRate` | Rate/threshold must be within `(0, 1]`. Use `FaultInjectionRule.Disable()` rather than a rate of 0. |
| `SetInjectionRate` appears to have no effect | The rule is a connection error rule, which uses `WithThreshold` (fixed at build time) instead of an injection rate. `SetInjectionRate` is a no-op for those rules. |
