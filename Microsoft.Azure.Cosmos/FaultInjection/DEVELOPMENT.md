...
| `ArgumentOutOfRangeException` on `WithInjectionRate` / `WithThreshold` / `SetInjectionRate` | Rate/threshold must be a number within `(0, 1]`; `double.NaN` and infinities are rejected. Use `FaultInjectionRule.Disable()` rather than a rate of 0. |
| `InvalidOperationException` from `SetInjectionRate` | The rule is a connection error rule, which uses `WithThreshold` (fixed at build time) instead of an injection rate. |
