    ...
    public class CancellationTokenTests
    {
        ...
        public async Task CancellationTokenDoesNotCancelFailover()
        {
            using (CancellationTokenSource source = new CancellationTokenSource())
            {
                ...
                ((MockDocumentClient)client.DocumentClient).MockGlobalEndpointManager.Verify(gep => gep.RefreshLocationAsync(It.IsAny<AccountProperties>(), It.Is<bool>(refresh => refresh == false)), Times.Once, "Should had marked the endpoint unavailable");
            }
        }
