...
namespace Microsoft.Azure.Cosmos.Tests.Routing
{
    ...
    public class AsyncCacheNonBlockingTests
    {
        ...
        /// <summary>
        /// Test to validate that when an exception is thrown during RefreshAsync() operation
        /// the cache removes the key for which a refresh was requested.
        /// </summary>
        [TestMethod]
        [Owner("dkunda")]
        public async Task RefreshAsync_WhenThrowsException_ShouldRemoveKeyFromTheCache()
        {
            // Arrange.
            AsyncCacheNonBlocking<string, string> asyncCache = new ();
            ...
            // this point and update the value to test3.
            result = await asyncCache.GetAsync(
                "key",
                (_) => Task.FromResult("value2"),
                (_) => false);
