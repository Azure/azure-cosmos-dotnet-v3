...
namespace Microsoft.Azure.Cosmos.ChangeFeed.Tests
{
    ...
    public class PartitionControllerSplitTests
    {
        ...
        public async Task Controller_ShouldSignalSynchronizerSplitPartition_IfPartitionSplitHappened()
        {
            ...
            PartitionSupervisorFactory partitionSupervisorFactory = Mock.Of<PartitionSupervisorFactory>(f => f.Create(lease, ChangeFeedMode.LatestVersion) == partitionSupervisor);
            ...
            PartitionControllerCore sut = new PartitionControllerCore(leaseContainer, leaseManager, partitionSupervisorFactory, synchronizer, Mock.Of<ChangeFeedProcessorHealthMonitor>(), ChangeFeedMode.LatestVersion);
        ...
        public async Task Controller_ShouldRunProcessingOnChildPartitions_IfHappyPath()
        {
            ...
            PartitionSupervisorFactory partitionSupervisorFactory = Mock.Of<PartitionSupervisorFactory>(f =>
                f.Create(lease, ChangeFeedMode.LatestVersion) == partitionSupervisor && f.Create(leaseChild1, ChangeFeedMode.LatestVersion) == partitionSupervisor1 && f.Create(leaseChild2, ChangeFeedMode.LatestVersion) == partitionSupervisor2);
            DocumentServiceLeaseManager leaseManager = Mock.Of<DocumentServiceLeaseManager>(manager => manager.AcquireAsync(lease) == Task.FromResult(lease));
            DocumentServiceLeaseContainer leaseContainer = Mock.Of<DocumentServiceLeaseContainer>();

            Mock<ChangeFeedProcessorHealthMonitor> monitor = new Mock<ChangeFeedProcessorHealthMonitor>();

            PartitionControllerCore sut = new PartitionControllerCore(leaseContainer, leaseManager, partitionSupervisorFactory, synchronizer, monitor.Object, ChangeFeedMode.LatestVersion);
            ...
            Mock.Get(partitionSupervisorFactory).Verify(f => f.Create(leaseChild1, ChangeFeedMode.LatestVersion), Times.Once);
            Mock.Get(partitionSupervisorFactory).Verify(f => f.Create(leaseChild2, ChangeFeedMode.LatestVersion), Times.Once);
        ...
        public async Task Controller_ShouldRunNotDeleteLease()
        {
            ...
            PartitionSupervisorFactory partitionSupervisorFactory = Mock.Of<PartitionSupervisorFactory>(f =>
                f.Create(lease, ChangeFeedMode.LatestVersion) == partitionSupervisor);
            DocumentServiceLeaseManager leaseManager = Mock.Of<DocumentServiceLeaseManager>(manager => manager.AcquireAsync(lease) == Task.FromResult(lease));
            DocumentServiceLeaseContainer leaseContainer = Mock.Of<DocumentServiceLeaseContainer>();
        ...
        public async Task Controller_ShouldIgnoreProcessingChildPartition_IfPartitionAlreadyAdded()
        {
            ...
            Mock.Get(partitionSupervisorFactory)
                .Verify(f => f.Create(leaseChild2, ChangeFeedMode.LatestVersion), Times.Once);
        }
