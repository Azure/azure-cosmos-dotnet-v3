...
namespace Microsoft.Azure.Cosmos.ChangeFeed.Tests
{
    ...
    public sealed class ChangeFeedObserverFactoryCoreTests
    {
        ...
        {
            bool executed = false;
            Task changesHandler(ChangeFeedProcessorContext context, IReadOnlyCollection<dynamic> docs, Func<Task<(bool isSuccess, CosmosException exception)>> tryCheckpointAsync, CancellationToken token)
            {
                Assert.AreEqual(1, docs.Count);
                Assert.AreEqual("Test", docs.First().id.ToString());
                executed = true;
        ...
        public async Task WhenDelegateIsStream_Manual()
        {
            ResponseMessage responseMessage = this.BuildResponseMessage();
            bool executed = false;
            Task changesHandler(ChangeFeedProcessorContext context, Stream stream, Func<Task<(bool isSuccess, CosmosException exception)>> tryCheckpointAsync, CancellationToken token)
            {
                Assert.ReferenceEquals(responseMessage.Content, stream);
                executed = true;
                return Task.CompletedTask;
