...
namespace Microsoft.Azure.Cosmos.Tests
{
    ...
    public class CosmosAuthorizationTests
    {
        ...
        // ---------------------------------------------------------------------------------------
        // Repro tests for the AAD ReadAccountAsync hang introduced by PR #5549 (CAE / token
        // revocation) between SDK 3.61.0 and main. ON THIS REPRO BRANCH (which does NOT contain the
        // fix) THESE TESTS ARE EXPECTED TO FAIL — that failure IS the reproduction. The fix is in
        // PR #6008 (branch nalutripician/readaccount-hang-repro); with it, all three pass.
        //
        // Root cause: TokenCredentialCache attaches a NON-EMPTY 'claims' parameter (the cp1 client
        // capability) on EVERY token acquisition — even when there is no revocation challenge —
        ...
        // correct, cache-friendly way via isCaeEnabled:true in CosmosScopeProvider.
        // ---------------------------------------------------------------------------------------
        ...
        public async Task TokenCredentialCache_NormalAcquisition_DoesNotStallOnMsalCacheBypass()
        {
            ...
            // stall. Before the fix, the SDK sends cp1 claims here -> the credential stalls -> the
