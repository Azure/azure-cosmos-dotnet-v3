namespace Microsoft.Azure.Cosmos.Tests
{
    ...
    public class AvailabilityStrategyUnitTests
    {
        ...
        /// <summary>
        /// Regression test proving the residual unobserved-task-exception crash mode is closed by the
        ...
        /// list and <c>continue</c>s. Before the fix, the faulted task was neither awaited nor had its
        ...
        /// <see cref="TaskScheduler.UnobservedTaskException"/>, crashing the host — the very failure mode
        /// this strategy is meant to eliminate.
        ///
        ...
        /// <summary>
        /// Regression test proving the broad
        /// <c>catch (Exception ex) when (hedgeRequestsCancellationTokenSource.IsCancellationRequested)</c>
        /// normalization does NOT over-reach. That filter must absorb a losing arm's exception ONLY
        /// after the hedge cancellation token source has been signalled (another region already won or
        /// the app hit its e2e timeout). When a hedge arm throws a genuine error while the hedge CTS is
        /// NOT cancelled and the application token is NOT cancelled, the original exception — not a
        /// <see cref="CosmosOperationCanceledException"/> — must propagate to the caller.
        /// </summary>
        [TestMethod]
        public async Task GenuineErrorWhileHedgeCtsNotCancelled_OriginalExceptionSurfaces()
        {
            ...
            Assert.AreEqual(
                sentinelMessage,
                caught.Message,
                "The genuine error must propagate unchanged — the post-cancellation normalization " +
                "filter must not fire while the hedge CTS is uncancelled.");
