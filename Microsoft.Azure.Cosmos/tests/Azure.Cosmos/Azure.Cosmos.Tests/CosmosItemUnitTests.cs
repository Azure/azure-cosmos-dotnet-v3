...
namespace Azure.Cosmos.Tests
{
    ...
    public class CosmosItemUnitTests
    {
        ...
        //        using (Response streamResponse = await container.CreateItemStreamAsync(
            ...
            ItemRequestOptions requestOptions = null)
        {
            TestHandler testHandler = new TestHandler((request, cancellationToken) =>
            {
                ...
                JToken.Parse(Microsoft.Azure.Documents.Routing.PartitionKeyInternal.Undefined.ToString());
                Assert.IsTrue(new JTokenEqualityComparer().Equals(
                        JToken.Parse(Microsoft.Azure.Documents.Routing.PartitionKeyInternal.Undefined.ToString()),
                        JToken.Parse(request.CosmosHeaders.PartitionKey)),
                        "Arguments {0} {1} ", Microsoft.Azure.Documents.Routing.PartitionKeyInternal.Undefined.ToString(), request.CosmosHeaders.PartitionKey);
            ...
            ItemRequestOptions requestOptions = null)
        {
            ...
            Assert.AreEqual((int)httpStatusCode, itemResponse.GetRawResponse().Status);
            ...
            using (Stream itemStream = MockCosmosUtil.Serializer.ToStream<dynamic>(testItem))
            {
                    ...
                    requestOptions: requestOptions))
                {
                    ...
                    Assert.AreEqual((int)httpStatusCode, streamResponse.Status);
                }
            }

            using (Stream itemStream = jsonSerializer.ToStream<dynamic>(testItem))
            {
                using (Response streamResponse = await container.ReadItemStreamAsync(
                    partitionKey: partitionKey,
                    id: testItem.id,
                    requestOptions: requestOptions))
                {
            ...
            using (Stream itemStream = jsonSerializer.ToStream<dynamic>(testItem))
            {
                using (Response streamResponse = await container.UpsertItemStreamAsync(
                    partitionKey: partitionKey,
                    streamPayload: itemStream,
                    requestOptions: requestOptions))
                {
            ...
            using (Stream itemStream = jsonSerializer.ToStream<dynamic>(testItem))
            {
                using (Response streamResponse = await container.ReplaceItemStreamAsync(
                    partitionKey: partitionKey,
                    id: testItem.id,
                    streamPayload: itemStream,
                    requestOptions: requestOptions))
            ...
            using (Stream itemStream = jsonSerializer.ToStream<dynamic>(testItem))
            {
                using (Response streamResponse = await container.DeleteItemStreamAsync(
                    partitionKey: partitionKey,
                    id: testItem.id,
                    requestOptions: requestOptions))
                {
