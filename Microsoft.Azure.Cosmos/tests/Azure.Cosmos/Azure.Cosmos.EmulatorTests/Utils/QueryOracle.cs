...
namespace Azure.Cosmos.EmulatorTests
{
    ...
    public sealed class QueryOracle
    {
        ...
        {
            Trace.TraceInformation("Using collection {0}", this.container.Id);

            DateTime startTime = DateTime.Now;
            this.ReadAllDocsAndBuildInvertedIndex().GetAwaiter().GetResult();
        ...
        private async Task ReadAllDocsAndBuildInvertedIndex()
        {
            ...
            await foreach (Page<JToken> page in iterator)
            {
                ...
                foreach (JToken doc in page.Values)
                {
        ...
        private async Task<int> QueryAndVerifyDocuments(int pageSize = 1000)
        {
            ...
            foreach (KeyValuePair<SqlKeyValueQueryBuilder, HashSet<string>> keyVal in this.invertedIndex)
            {
                ...
                await foreach (Page<dynamic> queryResultsPage in docQuery)
                {
                    ...
                    string activityId = queryResultsPage.GetRawResponse().Headers.GetActivityId();
                    activityIDsAllQueryPages.Add(activityId);
                    ...
                    foreach (JObject result in queryResultsPage.Values)
                    {
                        ...
                        if (!keyVal.Value.Contains(id))
                        {
                            ...
                            Trace.TraceInformation(
                                ...
                                @": The doc id {0} for query {1} was not expected in the results, query activityId: {2}", id, query, activityId);
                        ...
                        if (idSet.Contains(id))
                        {
                            Trace.TraceInformation(
                                DateTime.Now.ToString("HH:mm:ss.ffff") +
                                @": Same document id {0} returned twice for query ({1}), query activityId: {2}", id, query, activityId);
