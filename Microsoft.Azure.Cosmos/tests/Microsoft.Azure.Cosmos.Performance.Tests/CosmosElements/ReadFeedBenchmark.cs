    ...
    public class ReadFeedBenchmark
    {
        ...
        public void ParseByteArrayJson()
        {
            using (MemoryStream ms = new MemoryStream(this.payloadBytes))
            {
                ...
                ReadFeedResponse<ToDoActivity> persons = ReadFeedResponse<ToDoActivity>.CreateResponse<ToDoActivity>(responseMessage, this.serializerCore, Documents.ResourceType.Document, true);
        ...
        public void CosmosElements()
        {
            using (MemoryStream ms = new MemoryStream(this.payloadBytes))
            {
                ...
                ReadFeedResponse<ToDoActivity> persons = ReadFeedResponse<ToDoActivity>.CreateResponse<ToDoActivity>(responseMessage, this.serializerCore, Documents.ResourceType.Document, false);
                if (persons.Count != 1000)
                {
                    throw new Exception();
                }
