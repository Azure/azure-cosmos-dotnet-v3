...
namespace Microsoft.Azure.Cosmos.Services.Management.Tests.LinqProviderTests
{
    ...
    public class LinqTranslationBaselineTests : BaselineTests<LinqTestInput, LinqTestOutput>
    {
        ...
        public void TestUDFs()
        {
            ...
            List<LinqTestInput> inputs = new List<LinqTestInput>
            {
                new LinqTestInput("No param", b => getQuery(b).Select(f => CosmosLinq.Invoke("NoParameterUDF"))),
                new LinqTestInput("Single param", b => getQuery(b).Select(doc => CosmosLinq.Invoke("SingleParameterUDF", doc.NumericField))),
                new LinqTestInput("Single param w/ array", b => getQuery(b).Select(doc => CosmosLinq.Invoke("SingleParameterUDFWithArray", doc.ArrayField))),
                new LinqTestInput("Multi param", b => getQuery(b).Select(doc => CosmosLinq.Invoke("MultiParamterUDF", doc.NumericField, doc.StringField, doc.Point))),
                new LinqTestInput("Multi param w/ array", b => getQuery(b).Select(doc => CosmosLinq.Invoke("MultiParamterUDWithArrayF", doc.ArrayField, doc.NumericField, doc.Point))),
                new LinqTestInput("ArrayCount", b => getQuery(b).Where(doc => (int)CosmosLinq.Invoke("ArrayCount", doc.ArrayField) > 2)),
                new LinqTestInput("ArrayCount && SomeBooleanUDF", b => getQuery(b).Where(doc => (int)CosmosLinq.Invoke("ArrayCount", doc.ArrayField) > 2 && (bool)CosmosLinq.Invoke("SomeBooleanUDF"))),
                new LinqTestInput("expression", b => getQuery(b).Where(doc => (int)CosmosLinq.Invoke("SingleParameterUDF", doc.NumericField) + 2 == 4)),
                // UDF with constant parameters
                new LinqTestInput("Single constant param", b => getQuery(b).Select(doc => CosmosLinq.Invoke("SingleParameterUDF", 1))),
                new LinqTestInput("Single constant int array param", b => getQuery(b).Select(doc => CosmosLinq.Invoke("SingleParameterUDFWithArray", new int[] { 1, 2, 3 }))),
                new LinqTestInput("Single constant string array param", b => getQuery(b).Select(doc => CosmosLinq.Invoke("SingleParameterUDFWithArray", new string[] { "1", "2" }))),
                new LinqTestInput("Multi constant params", b => getQuery(b).Select(doc => CosmosLinq.Invoke("MultiParamterUDF", 1, "str", true))),
                new LinqTestInput("Multi constant array params", b => getQuery(b).Select(doc => CosmosLinq.Invoke("MultiParamterUDWithArrayF", new int[] { 1, 2, 3 }, 1, "str"))),
                new LinqTestInput("ArrayCount with constant param", b => getQuery(b).Where(doc => (int)CosmosLinq.Invoke("ArrayCount", new int[] { 1, 2, 3 }) > 2)),
                // regression (different type parameters including objects)
                new LinqTestInput("different type parameters including objects", b => getQuery(b).Where(doc => (bool)CosmosLinq.Invoke("MultiParamterUDF2", doc.Point, "str", 1))),
                // errors
                new LinqTestInput("Null udf name", b => getQuery(b).Select(doc => CosmosLinq.Invoke(null))),
                new LinqTestInput("Empty udf name", b => getQuery(b).Select(doc => CosmosLinq.Invoke("")))
            };
            this.ExecuteTestSuite(inputs);
        }
