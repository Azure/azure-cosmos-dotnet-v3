    ...
    public class CosmosNotFoundTests
    {
        ...
        private async Task ContainerOperations(CosmosDatabase database, bool dbNotExist)
        {
            ...
            if (dbNotExist)
            {
                ...
                this.VerifyNotFoundResponse(await database.CreateContainerAsStreamAsync(newcontainerSettings, requestUnits: 500));
            }
        ...
        private async Task ItemOperations(CosmosContainer container, bool containerNotExist)
        {
            if (containerNotExist)
            {
                ...
                this.VerifyNotFoundResponse(await container.CreateItemAsStreamAsync(new PartitionKey(randomItem.pk), create));
