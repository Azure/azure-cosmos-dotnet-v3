...
namespace Microsoft.Azure.Cosmos.SDK.EmulatorTests
{
    ...
    public abstract class ClientTelemetryBaseTests : BaseCosmosClientHelper
    {
        ...
        public static void ClassInitialize(TestContext _)
        {
            ...
            ClientTelemetryBaseTests.ResetSystemUsageMonitor(true);
        }
        ...
        public void TestInitialize()
        {
            ...
            this.cosmosClientBuilder = this.GetBuilder()
                                                .WithApplicationPreferredRegions(ClientTelemetryBaseTests.preferredRegionList);
        }

        protected abstract Task<HttpResponseMessage> HttpHandlerRequestCallbackChecks(HttpRequestMessage request);

        protected abstract CosmosClientBuilder GetBuilder();

        private static void ResetSystemUsageMonitor(bool isTelemetryEnabled)
        {
            ClientTelemetryBaseTests.systemUsageMonitor?.Stop();
            ...
            ClientTelemetryBaseTests.systemUsageMonitor = SystemUsageMonitor.CreateAndStart(recorders);
        }
        ...
        {
            ClientTelemetryBaseTests.ResetSystemUsageMonitor(false);
        }
            ...
            bool isExpectedNetworkTelemetry = true)
        {
            ...
            {
                ClientTelemetryBaseTests.expectedMetricNameUnitMap.TryAdd(ClientTelemetryOptions.NumberOfTcpConnectionName, ClientTelemetryOptions.NumberOfTcpConnectionUnit);
            }
            else
            {
                ClientTelemetryBaseTests.expectedMetricNameUnitMap.Remove(ClientTelemetryOptions.NumberOfTcpConnectionName);
            }

            ClientTelemetryBaseTests.AssertAccountLevelInformation(
            ...
            ClientTelemetryBaseTests.AssertOperationLevelInformation(
            ...
            if(!string.IsNullOrEmpty(expectedCacheSource))
            {
                ...
                ClientTelemetryBaseTests.AssertCacheRefreshInfoInformation(
                  cacheRefreshInfoSet: cacheRefreshInfoSet,
                  expectedCacheSource: expectedCacheSource);
            }
           
            ClientTelemetryBaseTests.AssertSystemLevelInformation(actualSystemInformation, ClientTelemetryBaseTests.expectedMetricNameUnitMap);
            if (localCopyOfActualInfo.First().ConnectionMode == ConnectionMode.Direct.ToString().ToUpperInvariant())
            {
                ...
                {
                    ClientTelemetryBaseTests.AssertNetworkLevelInformation(actualRequestInformation);
                }
            }
            else
            {
