...
"
        ...
        private async Task TestOrderyByQueryAsync()
        {
            ...
            FeedIterator<MyObject> setIterator1 = container.GetItemsQuery<MyObject>(cosmosSqlQueryDefinition1, maxConcurrency: -1, maxItemCount: -1);

            QueryDefinition cosmosSqlQueryDefinition2 = new QueryDefinition("SELECT * FROM root ORDER BY root[\"" + numberFieldName + "\"] DESC");
            FeedIterator<MyObject> setIterator2 = container.GetItemsQuery<MyObject>(cosmosSqlQueryDefinition2, maxConcurrency: -1, maxItemCount: -1);
