    ...
    public abstract class CustomSerializationTests
    {
        ...
        public async Task  TestJsonSerializerSettings(bool useGateway)
        {
            ...
            FeedIterator<TestDocument> feedIterator =
               container.GetItemsQueryIterator<TestDocument>(sqlQueryDefinition: sql, partitionKey: new Cosmos.PartitionKey(testDocument.Name), maxItemCount: 1);
...
"
        ...
        private async Task TestOrderyByQueryAsync()
        {
            ...
            FeedIterator<MyObject> setIterator1 = container.GetItemsQueryIterator<MyObject>(cosmosSqlQueryDefinition1, maxConcurrency: -1, maxItemCount: -1);

            QueryDefinition cosmosSqlQueryDefinition2 = new QueryDefinition("SELECT * FROM root ORDER BY root[\"" + numberFieldName + "\"] DESC");
            FeedIterator<MyObject> setIterator2 = container.GetItemsQueryIterator<MyObject>(cosmosSqlQueryDefinition2, maxConcurrency: -1, maxItemCount: -1);
