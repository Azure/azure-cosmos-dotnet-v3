...
namespace Microsoft.Azure.Cosmos.SDK.EmulatorTests
{
    ...
    public class CrossPartitionQueryTests
    {
            ...
            QueryRequestOptions queryRequestOptions = null)
        {
            ...
            do
            {
                FeedIterator<T> itemQuery = container.GetItemsQuery<T>(
                   sqlQueryText: query,
                   maxConcurrency: 2,
        ...
        private async Task TestBadQueriesOverMultiplePartitionsHelper(Container container, IEnumerable<Document> documents)
        {
            ...
            try
            {
                FeedIterator<Document> resultSetIterator = container.GetItemsQuery<Document>(
            ...
            IEnumerable<Document> documents)
        {
            ...
            FeedIterator<dynamic> resultSetIterator = container.GetItemsQuery<dynamic>(
            ...
            resultSetIterator = container.GetItemsQuery<dynamic>(
        ...
        private async Task TestQueryWithSpecialPartitionKeysHelper(Container container, IEnumerable<Document> documents, QueryWithSpecialPartitionKeysArgs testArgs)
        {
            ...
            returnedDoc = (await container.GetItemsQuery<SpecialPropertyDocument>(
                query,
        ...
        private async Task TestQueryCrossPartitionWithLargeNumberOfKeysHelper(Container container, IEnumerable<Document> documents, QueryCrossPartitionWithLargeNumberOfKeysArgs args)
        {
            ...
            FeedIterator<int> documentQuery = container.GetItemsQuery<int>(
        ...
        private async Task TestQueryCrossPartitionAggregateFunctionsAsync(Container container, IEnumerable<Document> documents, AggregateTestArgs aggregateTestArgs)
        {
            ...
            foreach (int maxDoP in new[] { 0, 10 })
            {
                ...
                foreach (Tuple<string, object> data in datum)
                {
                    ...
                    FeedResponse<dynamic> page = await container.GetItemsQuery<dynamic>(query, maxConcurrency: 1).ReadNextAsync();
                }
            }
        }
        ...
        private async Task TestQueryDistinct(Container container, IEnumerable<Document> documents, dynamic testArgs = null)
        {
            ...
            foreach (string query in queries)
            {
                foreach (int pageSize in new int[] { 1, 10, 100 })
                {
                    ...
                    FeedIterator<JToken> documentQueryWithoutDistinct = container.GetItemsQuery<JToken>(
                    ...
                    FeedIterator<JToken> documentQueryWithDistinct = container.GetItemsQuery<JToken>(
                        queryWithDistinct,
                        maxConcurrency: 100,
                        maxItemCount: pageSize);
            ...
            })
            {
                ...
                try
                {
                    ...
                    do
                    {
                        FeedIterator<JToken> documentQuery = container.GetItemsQuery<JToken>(
                            queryWithDistinct,
                            maxItemCount: 10,
                            maxConcurrency: 100);
            ...
            })
            {
                foreach (int pageSize in new int[] { 1, 10, 100 })
                {
                    ...
                    do
                    {
                        FeedIterator<JToken> cosmosQuery = container.GetItemsQuery<JToken>(
                                   sqlQueryText: queryWithDistinct,
                                   maxConcurrency: 100,
                                   maxItemCount: 1,
                                   continuationToken: continuationToken);
                        ...
                        foreach (bool hasTop in new[] { false, true })
                        {
                            foreach (bool hasOrderBy in new[] { false, true })
                            {
                                foreach (string sortOrder in new[] { string.Empty, "ASC", "DESC" })
                                {
                                    ...
                                    List<Document> result = new List<Document>();
                                    FeedIterator<Document> query = container.GetItemsQuery<Document>(
                                        querySpec,
                                        maxConcurrency: maxDegreeOfParallelism,
                ...
                foreach (bool useDistinct in new bool[] { true, false })
                {
                    foreach (int topCount in new int[] { 0, 1, 10 })
                    {
                        foreach (int pageSize in new int[] { 1, 10 })
                        {
                            ...
                            FeedIterator<dynamic> documentQuery = container.GetItemsQuery<dynamic>(
                                    query,
                                    maxConcurrency: 0,
                                    maxItemCount: pageSize);
        ...
        private async Task TestQueryCrossPartitionWithContinuationsHelper(Container container, IEnumerable<Document> documents, CrossPartitionWithContinuationsArgs args)
        {
            ...
            try
            {
                await container.GetItemsQuery<Document>(
                    "SELECT * FROM t",
                    maxConcurrency: 1,
            ...
            FeedResponse<Document> responseWithEmptyContinuationExpected = await container.GetItemsQuery<Document>(
        ...
        //    FeedIterator<dynamic> docQuery = coll.Items.GetItemsQuery<dynamic>(query, feedOptions);
        ...
        {
            FeedIterator<T> resultSetIterator = container.GetItemsQuery<T>(
                query,
