...
namespace Microsoft.Azure.Cosmos.SDK.EmulatorTests
{
    ...
    public class CrossPartitionQueryTests
    {
        ...
        {
            FeedIterator<DatabaseProperties> allDatabases = client.GetDatabasesIterator();
            ...
            QueryRequestOptions queryRequestOptions = null)
        {
            ...
            do
            {
                FeedIterator<T> itemQuery = container.GetItemsQueryIterator<T>(
                   sqlQueryText: query,
                   maxConcurrency: 2,
        ...
        private async Task TestBadQueriesOverMultiplePartitionsHelper(Container container, IEnumerable<Document> documents)
        {
            ...
            try
            {
                FeedIterator<Document> resultSetIterator = container.GetItemsQueryIterator<Document>(
            ...
            IEnumerable<Document> documents)
        {
            ...
            FeedIterator<dynamic> resultSetIterator = container.GetItemsQueryIterator<dynamic>(
            ...
            resultSetIterator = container.GetItemsQueryIterator<dynamic>(
        ...
        private async Task TestQueryWithSpecialPartitionKeysHelper(Container container, IEnumerable<Document> documents, QueryWithSpecialPartitionKeysArgs testArgs)
        {
            ...
            returnedDoc = (await container.GetItemsQueryIterator<SpecialPropertyDocument>(
                query,
                partitionKey: new Cosmos.PartitionKey(args.ValueToPartitionKey),
                maxItemCount: 1).ReadNextAsync()).First();
        ...
        private async Task TestQueryCrossPartitionWithLargeNumberOfKeysHelper(Container container, IEnumerable<Document> documents, QueryCrossPartitionWithLargeNumberOfKeysArgs args)
        {
            ...
            FeedIterator<int> documentQuery = container.GetItemsQueryIterator<int>(
        ...
        private async Task TestQueryCrossPartitionAggregateFunctionsAsync(Container container, IEnumerable<Document> documents, AggregateTestArgs aggregateTestArgs)
        {
            ...
            foreach (int maxDoP in new[] { 0, 10 })
            {
                ...
                foreach (Tuple<string, object> data in datum)
                {
                    ...
                    FeedResponse<dynamic> page = await container.GetItemsQueryIterator<dynamic>(query, maxConcurrency: 1).ReadNextAsync();
                }
            }
        }
        ...
        private async Task TestQueryDistinct(Container container, IEnumerable<Document> documents, dynamic testArgs = null)
        {
            ...
            foreach (string query in queries)
            {
                foreach (int pageSize in new int[] { 1, 10, 100 })
                {
                    ...
                    FeedIterator<JToken> documentQueryWithoutDistinct = container.GetItemsQueryIterator<JToken>(
                    ...
                    FeedIterator<JToken> documentQueryWithDistinct = container.GetItemsQueryIterator<JToken>(
                        queryWithDistinct,
                        maxConcurrency: 100,
                        maxItemCount: pageSize);
            ...
            })
            {
                ...
                try
                {
                    ...
                    do
                    {
                        FeedIterator<JToken> documentQuery = container.GetItemsQueryIterator<JToken>(
                            queryWithDistinct,
                            maxItemCount: 10,
                            maxConcurrency: 100);
            ...
            })
            {
                foreach (int pageSize in new int[] { 1, 10, 100 })
                {
                    ...
                    do
                    {
                        FeedIterator<JToken> cosmosQuery = container.GetItemsQueryIterator<JToken>(
                                   sqlQueryText: queryWithDistinct,
                                   maxConcurrency: 100,
                                   maxItemCount: 1,
                                   continuationToken: continuationToken);
                        ...
                        foreach (bool hasTop in new[] { false, true })
                        {
                            foreach (bool hasOrderBy in new[] { false, true })
                            {
                                foreach (string sortOrder in new[] { string.Empty, "ASC", "DESC" })
                                {
                                    ...
                                    List<Document> result = new List<Document>();
                                    FeedIterator<Document> query = container.GetItemsQueryIterator<Document>(
                                        querySpec,
                                        maxConcurrency: maxDegreeOfParallelism,
                ...
                foreach (bool useDistinct in new bool[] { true, false })
                {
                    foreach (int topCount in new int[] { 0, 1, 10 })
                    {
                        foreach (int pageSize in new int[] { 1, 10 })
                        {
                            ...
                            FeedIterator<dynamic> documentQuery = container.GetItemsQueryIterator<dynamic>(
                                    query,
                                    maxConcurrency: 0,
                                    maxItemCount: pageSize);
        ...
        private async Task TestQueryCrossPartitionWithContinuationsHelper(Container container, IEnumerable<Document> documents, CrossPartitionWithContinuationsArgs args)
        {
            ...
            try
            {
                await container.GetItemsQueryIterator<Document>(
                    "SELECT * FROM t",
                    maxConcurrency: 1,
                    continuationToken: Guid.NewGuid().ToString()).ReadNextAsync();
            ...
            try
            {
                await container.GetItemsQueryIterator<Document>(
                    ...
                    maxConcurrency: -1,
                    maxItemCount: 10,
                    continuationToken: "{'top':11}").ReadNextAsync();
            ...
            try
            {
                await container.GetItemsQueryIterator<Document>(
                    ...
                    maxConcurrency: -1,
                    maxItemCount: 10,
                    continuationToken: "{'compositeToken':{'range':{'min':'05C1E9CD673398','max':'FF'}}, 'orderByItems':[{'item':2}, {'item':1}]}").ReadNextAsync();
            ...
            FeedResponse<Document> responseWithEmptyContinuationExpected = await container.GetItemsQueryIterator<Document>(
                string.Format(CultureInfo.InvariantCulture, "SELECT TOP 1 * FROM r ORDER BY r.{0}", partitionKey),
                    maxConcurrency: 10,
                    maxItemCount: -1).ReadNextAsync();
        ...
        //    FeedIterator<dynamic> docQuery = coll.Items.GetItemsQueryIterator<dynamic>(query, feedOptions);
        ...
        {
            FeedIterator<T> resultSetIterator = container.GetItemsQueryIterator<T>(
                query,
