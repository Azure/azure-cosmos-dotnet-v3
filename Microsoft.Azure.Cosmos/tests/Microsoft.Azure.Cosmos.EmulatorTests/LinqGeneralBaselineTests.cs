...
namespace Microsoft.Azure.Cosmos.Services.Management.Tests.LinqProviderTests
{
    ...
    public class LinqGeneralBaselineTests : BaselineTests<LinqTestInput, LinqTestOutput>
    {
        ...
        public void TestSelectMany()
        {
            ...
            {
                new LinqTestInput("SelectMany(SelectMany(Where -> Select))",
                b => getQuery(b)
                ...
                new LinqTestInput("SelectMany(Where -> SelectMany(Where -> Select))",
                b => getQuery(b)
                ...
                new LinqTestInput("SelectMany(SelectMany(Where -> Select new {}))",
                b => getQuery(b)
                .SelectMany(family => family.Children
                    .SelectMany(child => child.Pets
                        .Where(pet => pet.GivenName == "Fluffy")
                ...
                new LinqTestInput("SelectMany()", b => getQuery(b).SelectMany(root => root.Children)),

                new LinqTestInput("SelectMany -> SelectMany", b => getQuery(b).SelectMany(f => f.Children).SelectMany(c => c.Pets)),

                new LinqTestInput("SelectMany -> Where -> SelectMany(Select)", b => getQuery(b).SelectMany(f => f.Children).Where(c => c.Pets.Count() > 0).SelectMany(c => c.Pets.Select(p => p.GivenName))),

                new LinqTestInput("SelectMany -> Where -> SelectMany(Select new)", b => getQuery(b)
                    .SelectMany(f => f.Children)
                    .Where(c => c.Pets.Count() > 0)
                    .SelectMany(c => c.Pets.Select(p => new { PetName = p.GivenName, OwnerName = c.GivenName }))),

                new LinqTestInput("Where -> SelectMany", b => getQuery(b).Where(f => f.Children.Count() > 0).SelectMany(f => f.Children)),

                new LinqTestInput("SelectMany -> Select", b => getQuery(b).SelectMany(f => f.Children).Select(c => c.FamilyName)),

                new LinqTestInput("SelectMany(Select)", b => getQuery(b).SelectMany(f => f.Children.Select(c => c.Pets.Count()))),

                new LinqTestInput("SelectMany(Select)", b => getQuery(b).SelectMany(f => f.Parents.Select(p => p.FamilyName))),

                new LinqTestInput("SelectMany(Select -> Select)", b => getQuery(b).SelectMany(f => f.Parents.Select(p => p.FamilyName).Select(n => n.Count()))),

                new LinqTestInput("SelectMany(Select -> Where)", b => getQuery(b).SelectMany(f => f.Parents.Select(p => p.FamilyName).Where(n => n.Count() > 10))),

                new LinqTestInput("SelectMany(Take -> Where)", b => getQuery(b).SelectMany(f => f.Children.Take(2).Where(c => c.FamilyName.Count() > 10))),

                new LinqTestInput("SelectMany(OrderBy -> Take -> Where)", b => getQuery(b).SelectMany(f => f.Children.OrderBy(c => c.Grade).Take(2).Where(c => c.FamilyName.Count() > 10))),

                new LinqTestInput("SelectMany(Distinct -> Where)", b => getQuery(b).SelectMany(f => f.Children.Distinct().Where(c => c.FamilyName.Count() > 10)))
            };
        ...
        public void TestSimpleSubquery()
        {
            List<LinqTestInput> inputs = new List<LinqTestInput>
            {
                new LinqTestInput("Select -> Select", b => getQuery(b).Select(f => f.FamilyId).Select(n => n.Count())),

                new LinqTestInput("Select -> Where", b => getQuery(b).Select(f => f.FamilyId).Where(id => id.Count() > 10)),

                new LinqTestInput("Select -> OrderBy -> Take -> Select -> Orderby -> Take", b => getQuery(b).Select(x => x).OrderBy(x => x).Take(10).Select(f => f.FamilyId).OrderBy(n => n.Count()).Take(5)),

                new LinqTestInput("Select -> Orderby -> Take -> Select -> Orderby -> Take", b => getQuery(b).Select(f => f).OrderBy(f => f.Children.Count()).Take(3).Select(x => x).OrderBy(f => f.Parents.Count()).Take(2)),

                new LinqTestInput("Orderby -> Take -> Orderby -> Take", b => getQuery(b).OrderBy(f => f.Children.Count()).Take(3).OrderBy(f => f.Parents.Count()).Take(2)),

                new LinqTestInput("Take -> Orderby -> Take", b => getQuery(b).Take(10).OrderBy(f => f.FamilyId).Take(1)),

                new LinqTestInput("Take -> Where -> Take -> Where -> Take -> Where", b => getQuery(b).Take(10).Where(f => f.Children.Count() > 0).Take(9).Where(f => f.Parents.Count() > 0).Take(8).Where(f => f.FamilyId.Count() > 10)),

                new LinqTestInput("Take -> Where -> Distinct -> Select -> Take -> Where", b => getQuery(b).Take(10).Where(f => f.Children.Count() > 0).Distinct().Select(f => new { f }).Take(8).Where(f => f.f.FamilyId.Count() > 10)),

                new LinqTestInput("Distinct -> Select -> Take -> Where -> Take -> Where", b => getQuery(b).Distinct().Select(f => new { f }).Take(10).Where(f => f.f.Children.Count() > 0).Take(9).Where(f => f.f.Parents.Count() > 0))
            };
        ...
        public void TestQueryFlattening()
        {
            ...
            List<LinqTestInput> inputs = new List<LinqTestInput>
            {
                new LinqTestInput("array create", b => getQuery(b).Select(f => f.Int).Select(i => new int[] { i })),
                new LinqTestInput("unary operation", b => getQuery(b).Select(f => f.Int).Select(i => -i)),
                new LinqTestInput("binary operation", b => getQuery(b).Select(f => f).Select(i => i.Int % 10 * i.Children.Count())),
                new LinqTestInput("literal", b => getQuery(b).Select(f => f.Int).Select(i => 0)),
                new LinqTestInput("function call", b => getQuery(b).Select(f => f.Parents).Select(p => p.Count())),
                new LinqTestInput("object create", b => getQuery(b).Select(f => f.Parents).Select(p => new { parentCount = p.Count() })),
                new LinqTestInput("conditional", b => getQuery(b).Select(f => f.Children).Select(c => c.Count() > 0 ? "have kids" : "no kids")),
                new LinqTestInput("property ref + indexer", b => getQuery(b).Select(f => f)
                    .Where(f => f.Children.Count() > 0 && f.Children[0].Pets.Count() > 0)
                    .Select(f => f.Children[0].Pets[0].GivenName)),

                new LinqTestInput("array creation -> indexer", b => getQuery(b).Select(f => new int[] { f.Int }).Select(array => array[0])),
                new LinqTestInput("unary, indexer, property, function call -> function call", b => getQuery(b)
                    .Where(f => f.Children.Count() > 0)
                    .Select(f => -f.Children[0].Pets.Count()).Select(i => Math.Abs(i))),
                new LinqTestInput("binary operation, function call -> conditional", b => getQuery(b).Select(i => i.Int % 10 * i.Children.Count()).Select(i => i > 0 ? new int[] { i } : new int[] { })),
                new LinqTestInput("object creation -> conditional", b => getQuery(b)
                ...
                new LinqTestInput("indexer -> function call", b => getQuery(b).Select(f => f.Parents[0]).Select(p => string.Concat(p.FamilyName, p.GivenName))),
                new LinqTestInput("conditional -> object creation", b => getQuery(b).Select(f => f.Parents.Count() > 0 ? f.Parents : new Parent[0]).Select(p => new { parentCount = p.Count() })),
                new LinqTestInput("object creation -> conditional", b => getQuery(b).Select(f => new { children = f.Children }).Select(c => c.children.Count() > 0 ? c.children[0].GivenName : "no kids")),
                new LinqTestInput("object creation -> conditional", b => getQuery(b).Select(f => new { family = f, children = f.Children.Count() }).Select(f => f.children > 0 && f.family.Children[0].Pets.Count() > 0 ? f.family.Children[0].Pets[0].GivenName : "no kids"))
            };
            this.ExecuteTestSuite(inputs);
        }
        ...
        public void TestUnsupportedScenarios()
        {
            List<LinqTestInput> inputs = new List<LinqTestInput>
            {
                ...
                new LinqTestInput("Iterating through Dictionary type", b => getQuery(b).Select(f => f.Children.Select(c => c.Things.Select(t => t.Key.Count() + t.Value.Count())))),

                // Get a count of a Dictionary type
                new LinqTestInput("Getting Dictionary count", b => getQuery(b).Select(f => f.Children.Select(c => c.Things.Count())))
            };
        ...
        public void TestOrderByTranslation()
        {
            List<LinqTestInput> inputs = new List<LinqTestInput>
            {
                ...
                // Ascending
                new LinqTestInput("Select -> order by", b => getQuery(b).Select(family => family.FamilyId).OrderBy(id => id)),
                new LinqTestInput("Select -> order by -> Select", b => getQuery(b).Select(family => family.FamilyId).OrderBy(id => id).Select(x => x.Count())),
                new LinqTestInput("Where -> OrderBy -> Select query",
                b => from f in getQuery(b)
                     where f.Int == 5 && f.NullableInt != null
                     orderby f.IsRegistered
                     select f.FamilyId),
                new LinqTestInput("Where -> OrderBy -> Select", b => getQuery(b).Where(f => f.Int == 5 && f.NullableInt != null).OrderBy(f => f.IsRegistered).Select(f => f.FamilyId)),
                new LinqTestInput("OrderBy query",
                b => from f in getQuery(b)
                     orderby f.FamilyId
                     select f),
                new LinqTestInput("OrderBy", b => getQuery(b).OrderBy(f => f.FamilyId)),
                new LinqTestInput("OrderBy -> Select query",
                b => from f in getQuery(b)
                     orderby f.FamilyId
                     select f.FamilyId),
                new LinqTestInput("OrderBy -> Select", b => getQuery(b).OrderBy(f => f.FamilyId).Select(f => f.FamilyId)),
                new LinqTestInput("OrderBy -> Select -> Take", b => getQuery(b).OrderBy(f => f.FamilyId).Select(f => f.FamilyId).Take(10)),
                new LinqTestInput("OrderBy -> Select -> Select", b => getQuery(b).OrderBy(f => f.FamilyId).Select(f => f.FamilyId).Select(x => x)),

                // Descending
                new LinqTestInput("Select -> order by", b => getQuery(b).Select(family => family.FamilyId).OrderByDescending(id => id)),
                new LinqTestInput("Select -> order by -> Select", b => getQuery(b).Select(family => family.FamilyId).OrderByDescending(id => id).Select(x => x.Count())),
                new LinqTestInput("Where -> OrderBy Desc -> Select query",
                b => from f in getQuery(b)
                     where f.Int == 5 && f.NullableInt != null
                     orderby f.IsRegistered descending
                     select f.FamilyId),
                new LinqTestInput("Where -> OrderBy Desc -> Select", b => getQuery(b).Where(f => f.Int == 5 && f.NullableInt != null).OrderByDescending(f => f.IsRegistered).Select(f => f.FamilyId)),
                new LinqTestInput("OrderBy Desc query",
                b => from f in getQuery(b)
                     orderby f.FamilyId descending
                     select f),
                new LinqTestInput("OrderBy Desc", b => getQuery(b).OrderByDescending(f => f.FamilyId)),
                new LinqTestInput("OrderBy Desc -> Select query",
                b => from f in getQuery(b)
                     orderby f.FamilyId descending
                     select f.FamilyId),
                new LinqTestInput("OrderBy Desc -> Select", b => getQuery(b).OrderByDescending(f => f.FamilyId).Select(f => f.FamilyId)),
                new LinqTestInput("OrderBy -> Select -> Take", b => getQuery(b).OrderByDescending(f => f.FamilyId).Select(f => f.FamilyId).Take(10)),
                new LinqTestInput("OrderBy -> Select -> Select", b => getQuery(b).OrderByDescending(f => f.FamilyId).Select(f => f.FamilyId).Select(x => x)),
                new LinqTestInput("OrderBy multiple expressions",
                b => from f in getQuery(b)
        ...
        public void ValidateDynamicLinq()
        {
            List<LinqTestInput> inputs = new List<LinqTestInput>
            {
                new LinqTestInput("Select", b => getQuery(b).Select("FamilyId")),
                new LinqTestInput("Where", b => getQuery(b).Where("FamilyId = \"some id\"")),
                new LinqTestInput("Where longer", b => getQuery(b).Where("FamilyId = \"some id\" AND IsRegistered = True OR Int > 101")),
                // with parameters
                new LinqTestInput("Where w/ parameters", b => getQuery(b).Where("FamilyId = @0 AND IsRegistered = @1 OR Int > @2", "some id", true, 101)),
                new LinqTestInput("Where -> Select", b => getQuery(b).Where("FamilyId = \"some id\"").Select("Int"))
            };
            this.ExecuteTestSuite(inputs);
        }
        ...
        public async Task ValidateLinqQueries()
        {
            ...
            List<LinqTestInput> inputs = new List<LinqTestInput>
            {
                new LinqTestInput("Select 1st parent family name", b => getQuery(b).Where(f => f.Parents.Count() > 0).Select(f => f.Parents[0].FamilyName)),
                new LinqTestInput("Select 1st children grade expr", b => getQuery(b).Where(f => f.Children.Count() > 0).Select(f => f.Children[0].Grade + 13)),
                new LinqTestInput("Filter 1st children's 1st pet name", b => getQuery(b).Where(f => f.Children.Count() > 0 && f.Children[0].Pets.Count() > 0 && f.Children[0].Pets[0].GivenName == "Fluffy")),
                new LinqTestInput("Filter 1st children's thing A value", b => getQuery(b).Where(f => f.Children.Count() > 0 && f.Children[0].Things["A"] == "B")),
                new LinqTestInput("Filter 1st children's gender -> Select his 1st pet", b => getQuery(b).Where(f => f.Children.Count() > 0 && f.Children[0].Pets.Count() > 0 && f.Children[0].Gender == "female").Select(f => f.Children[0].Pets[0])),
                new LinqTestInput("Filter 1st children's gender -> Select his 2nd pet", b => getQuery(b).Where(f => f.Children.Count() > 0 && f.Children[0].Pets.Count() > 1 && f.Children[0].Gender == "female").Select(f => f.Children[0].Pets[1])),
                new LinqTestInput("Select FamilyId of all children", b => getQuery(b).SelectMany(f => f.Children.Select(c => new { Id = f.FamilyId }))),
                new LinqTestInput("Filter family with null Id", b => getQuery(b).Where(f => null == f.FamilyId)),
                new LinqTestInput("Filter family with null Id #2", b => getQuery(b).Where(f => f.FamilyId == null)),
                new LinqTestInput("Filter family with null object", b => getQuery(b).Where(f => f.NullObject == null)),
                new LinqTestInput("Filter family with null Id #3", b => getQuery(b).Where(f => null == f.FamilyId)),
                new LinqTestInput("Filter registered family", b => getQuery(b).Where(f => f.IsRegistered == false)),
                new LinqTestInput("Filter family by FamilyId", b => getQuery(b).Where(f => f.FamilyId.Equals("WakefieldFamily"))),
                new LinqTestInput("Filter family nullable int", b => getQuery(b).Where(f => f.NullableInt == null)),
                new LinqTestInput("Filter family nullable int #2", b => getQuery(b).Where(f => f.NullableInt == nullIntVal)),
                new LinqTestInput("Filter family nullable int =", b => getQuery(b).Where(f => f.NullableInt == 5)),
                new LinqTestInput("Filter nullableInt = nullInt", b => getQuery(b).Where(f => f.NullableInt == nullableIntVal)),
                new LinqTestInput("Filter nullableInt = nullInt value", b => getQuery(b).Where(f => f.NullableInt == nullableIntVal.Value)),
                new LinqTestInput("Filter int = nullInt", b => getQuery(b).Where(f => f.Int == nullableIntVal)),
                new LinqTestInput("Filter int = nullInt value", b => getQuery(b).Where(f => f.Int == nullableIntVal.Value)),
                new LinqTestInput("Filter int = nullInt", b => getQuery(b).Where(f => f.Int == nullIntVal)),
                new LinqTestInput("Filter int < nullInt", b => getQuery(b).Where(f => f.Int < nullIntVal)),

                new LinqTestInput("Guid filter by Id", b => getGuidQuery(b).Where(g => g.Id == guidObject.Id)),
                new LinqTestInput("Guid filter by Id #2", b => getGuidQuery(b).Where(g => g.Id.ToString() == guidObject.Id.ToString())),
                new LinqTestInput("Array compare", b => getListArrayQuery(b).Where(a => a.ArrayField == arrayObject.ArrayField)),
                new LinqTestInput("Array compare null", b => getListArrayQuery(b).Where(a => a.ArrayField == nullArray)),
                new LinqTestInput("List compare", b => getListArrayQuery(b).Where(a => a.ListField == listObject.ListField)),

                new LinqTestInput("Nested great family query filter children name", b => getGreatFamilyQuery(b).SelectMany(gf => gf.GreatFamily.Family.Children.Where(c => c.GivenName == "Jesse").Select(c => gf))),
                new LinqTestInput("Sport filter sport name", b => getSportQuery(b).Where(s => s.SportName == "Tennis")),
                new LinqTestInput("Sport filter sport type", b => getSportQuery(b).Where(s => s.SportType == "Racquet")),
                new LinqTestInput("Sport2 filter by id", b => getSport2Query(b).Where(s => s.id == "json"))
            };
            this.ExecuteTestSuite(inputs);
        }
        ...
        public void ValidateDynamicDocumentQuery() //Ensure query on custom property of document.
        {
            Book myDocument = new Book
            {
                ...
                Languages = new Language[] { new Language { Name = "English", Copyright = "London Publication" }, new Language { Name = "French", Copyright = "Paris Publication" } }, //Array Property
                Author = new Author { Name = "Don", Location = "France" }, //Complex Property
                Price = 9.99,
                Editions = new List<Edition>() { new Edition() { Name = "First", Year = 2001 }, new Edition() { Name = "Second", Year = 2005 } }
            };
            ...
            List<LinqTestInput> inputs = new List<LinqTestInput>
            {
                new LinqTestInput("Simple Equality on custom property",
                b => from book in getBookQuery(b)
                     where book.Title == "My Book"
                     select book),
                new LinqTestInput("Nested Property access",
                b => from book in getBookQuery(b)
                     where book.Author.Name == "Don"
                     select book),
                new LinqTestInput("Array references & Project Author out..",
                b => from book in getBookQuery(b)
                     where book.Languages[0].Name == "English"
                     select book.Author),
                new LinqTestInput("SelectMany",
                b => getBookQuery(b).SelectMany(book => book.Languages).Where(lang => lang.Name == "French").Select(lang => lang.Copyright)),
                new LinqTestInput("NumericRange query",
                b => from book in getBookQuery(b)
                     where book.Price < 10
                     select book.Author),
                new LinqTestInput("Or query",
                b => from book in getBookQuery(b)
                     where book.Title == "My Book" || book.Author.Name == "Don"
                     select book),
                new LinqTestInput("SelectMany query on a List type.",
        ...
        public void ValidateDynamicAttachmentQuery() //Ensure query on custom property of attachment.
        {
            ...
            List<LinqTestInput> inputs = new List<LinqTestInput>
            {
                new LinqTestInput("Filter equality on custom property",
                b => from attachment in getAttachmentQuery(b)
                     where attachment.Title == "My Book Title2"
                     select attachment),
                new LinqTestInput("Filter equality on custom property #2",
                b => from attachment in getAttachmentQuery(b)
