    ...
    public class CrossPartitionQueryTestsOnePartition
    {
        ...
        private async Task<Tuple<CosmosContainer, List<Document>>> CreatePartitionedContainerAndIngestDocuments(IEnumerable<string> documents, string partitionKey = "/id", Cosmos.IndexingPolicy indexingPolicy = null)
        {
            ...
            foreach (string document in documents)
            {
                ...
                insertedDocuments.Add((await partitionedCollection.Items.CreateItemAsync<JObject>(documentObject, new ItemRequestOptions { PartitionKey = pkValue })).Resource.ToObject<Document>());
