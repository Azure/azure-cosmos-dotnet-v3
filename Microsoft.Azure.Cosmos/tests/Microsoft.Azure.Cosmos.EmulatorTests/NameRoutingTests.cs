...
namespace Microsoft.Azure.Cosmos.SDK.EmulatorTests
{
    ...
    // DIRECT MODE has ReadFeed issues in the Public emulator
            ...
            try
            {
                ...
                Document doc1;
                {
                    ...
                    // query documents 
                    {
                        ...
                        FeedIterator<Document> docServiceQuery = container.GetItemsQueryIterator<Document>(
                ...
                //8. StoredProcedure CRUD
                {
                    ...
                    // 
                    // read databaseCollection feed.
                    FeedIterator<StoredProcedureProperties> storedProcedureIter = scripts.GetStoredProceduresIterator();
        ...
        private async Task ReplaceDocumentWithUriPrivateAsync(CosmosClient client)
        {
            ...
            FeedIterator<LinqGeneralBaselineTests.Book> cosmosResultSet = collection.GetItemsQueryIterator<LinqGeneralBaselineTests.Book>(sqlQueryText: sqlQueryText, maxConcurrency : 1, maxItemCount: 1, requestOptions: new QueryRequestOptions { EnableCrossPartitionQuery = true });
            ...
            cosmosResultSet = collection.GetItemsQueryIterator<LinqGeneralBaselineTests.Book>(sqlQueryText: sqlQueryText, maxConcurrency: 1, maxItemCount: 1, requestOptions: new QueryRequestOptions { EnableCrossPartitionQuery = true });
            ...
            CallAPIForStaleCacheTest eApiTest)
        {
            ...
            {
                Container collFooRead = await collFoo2.ReadAsync();
            }
        ...
        private async Task VerifyInvalidPartitionExceptionWithPopulateQuotaInfo(CosmosClient client)
        {
            ...
            try
            {
                ...
                // 2. GW -> MC: call mc to resolve collectionFullName.
                // 3. MC : return NotFoundException
                ContainerResponse readContainerResponse = await containerResponse.Container.ReadAsync(requestOptions: new ContainerRequestOptions { PopulateQuotaInfo = true });
        ...
        private async Task<IList<Container>> CreateContainerssAsync(CosmosDatabase database, int numberOfCollectionsPerDatabase)
        {
            PartitionKeyDefinition partitionKeyDefinition = new PartitionKeyDefinition { Paths = new System.Collections.ObjectModel.Collection<string>(new[] { "/id" }), Kind = PartitionKind.Hash };
