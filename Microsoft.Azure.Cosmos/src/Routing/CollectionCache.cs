...
namespace Microsoft.Azure.Cosmos.Common
{
    ...
    internal abstract class CollectionCache
    {
        ...
        protected class InternalCache
        {
            ...
            {
                collectionInfoByName = new AsyncCache<string, CosmosContainerProperties>(new CollectionRidComparer());
                collectionInfoById = new AsyncCache<string, CosmosContainerProperties>(new CollectionRidComparer());
            ...
            internal readonly AsyncCache<string, CosmosContainerProperties> collectionInfoByName;
            internal readonly AsyncCache<string, CosmosContainerProperties> collectionInfoById;
        ...
        public virtual Task<CosmosContainerProperties> ResolveCollectionAsync(
        ...
        protected abstract Task<CosmosContainerProperties> GetByRidAsync(string apiVersion, string collectionRid, CancellationToken cancellationToken);

        protected abstract Task<CosmosContainerProperties> GetByNameAsync(string apiVersion, string resourceAddress, CancellationToken cancellationToken);

        private async Task<CosmosContainerProperties> ResolveByPartitionKeyRangeIdentityAsync(string apiVersion, PartitionKeyRangeIdentity partitionKeyRangeIdentity, CancellationToken cancellationToken)
        {
        ...
        private Task<CosmosContainerProperties> ResolveByRidAsync(
            string apiVersion,
            string resourceId,
            CancellationToken cancellationToken)
        {
            ...
            return cache.collectionInfoById.GetAsync(
                ...
                {
                    DateTime currentTime = DateTime.UtcNow;
                    CosmosContainerProperties collection = await this.GetByRidAsync(apiVersion, collectionResourceId, cancellationToken);
        ...
        private sealed class CollectionRidComparer : IEqualityComparer<CosmosContainerProperties>
        {
            public bool Equals(CosmosContainerProperties left, CosmosContainerProperties right)
            {
            ...
            public int GetHashCode(CosmosContainerProperties collection)
            {
