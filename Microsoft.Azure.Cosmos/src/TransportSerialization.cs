...
namespace Microsoft.Azure.Cosmos
{
    ...
    using RequestPool = Microsoft.Azure.Documents.RntbdConstants.RntbdEntityPool<Microsoft.Azure.Documents.RntbdConstants.Request, Microsoft.Azure.Documents.RntbdConstants.RequestIdentifiers>;
    ...
    internal static class TransportSerialization
    {
        ...
        /// <summary>
        /// Builds the RNTBD request needed for the Photon (ThinClient) protocol
        /// </summary>
        internal static SerializedRequest BuildRequestForProxy(
            ...
            out int? bodySize)
        {
            ...
            using ProxyRequest.ObjectPool.EntityOwner owner = ProxyRequest.ObjectPool.Instance.Get();
            RntbdConstants.Request rntbdRequest = owner.Entity.RntbdRequest;
            ...
            return BuildRequestCore(
                request,
                ref rntbdRequest,
                replicaPathForDiagnostics: "thinClientReplica",
                resourceOperation,
                activityId,
                bufferProvider,
                out headerSize,
        ...
        private static SerializedRequest BuildRequestCore(
            DocumentServiceRequest request,
            ref RntbdConstants.Request rntbdRequest,
            string replicaPathForDiagnostics,
            ResourceOperation resourceOperation,
            Guid activityId,
            BufferProvider bufferProvider,
            out int headerSize,
            out int? bodySize)
        {
            ...
            TransportSerialization.AddIsServerlessEnabledStorageRefreshRequest(requestHeaders, rntbdRequest);
#endif
            ...
            TransportSerialization.AddSupportedSerializationFormats(requestHeaders, rntbdRequest);
            ...
            TransportSerialization.AddUpdateOfferStateToPendingForThroughputSplit(requestHeaders, rntbdRequest);
            ...
            TransportSerialization.FillTokenFromHeader(request, HttpConstants.HttpHeaders.PruneCollectionSchemas, requestHeaders.PruneCollectionSchemas, rntbdRequest.pruneCollectionSchemas, rntbdRequest);
            TransportSerialization.FillTokenFromHeader(request, HttpConstants.HttpHeaders.IsMigratedFixedCollection, requestHeaders.IsMigratedFixedCollection, rntbdRequest.isMigratedFixedCollection, rntbdRequest);
            TransportSerialization.FillTokenFromHeader(request, WFConstants.BackendHeaders.PopulateMinGLSNForDocumentOperations, requestHeaders.PopulateMinGLSNForDocumentOperations, rntbdRequest.populateMinGLSNForDocumentOperations, rntbdRequest);
            TransportSerialization.FillTokenFromHeader(request, WFConstants.BackendHeaders.PopulateHighestTentativeWriteLLSN, requestHeaders.PopulateHighestTentativeWriteLLSN, rntbdRequest.populateHighestTentativeWriteLLSN, rntbdRequest);
            ...
            TransportSerialization.FillTokenFromHeader(request, WFConstants.BackendHeaders.IsRequestFromComputeNotAuthorized, requestHeaders.IsRequestFromComputeNotAuthorized, rntbdRequest.isRequestFromComputeNotAuthorized, rntbdRequest);
            ...
            if (headerSize > HeaderSizeWarningThreshold)
            {
                DefaultTrace.TraceWarning(
                    ...
                    request.ResourceType, resourceOperation, replicaPathForDiagnostics);
            }
        ...
        internal static RntbdConstants.RntbdOperationType GetRntbdOperationType(OperationType operationType)
        {
            ...
            case OperationType.CreateRidRangeResources:
                return RntbdConstants.RntbdOperationType.CreateRidRangeResources;
        ...
        {
            if (request.Properties != null && request.Properties.TryGetValue(WFConstants.BackendHeaders.BinaryId, out object binaryPayload))
            {
                if (binaryPayload is byte[] binaryBytesData)
                {
                    rntbdRequest.binaryId.value.valueBytes = binaryBytesData;
                }
#if COSMOSCLIENT
                else if (binaryPayload is ReadOnlyMemory<byte> binaryRomBytesData)
                {
                    rntbdRequest.binaryId.value.valueBytes = binaryRomBytesData;
                }
#endif
                else
                {
        ...
        private static void AddEffectivePartitionKeyIfPresent(DocumentServiceRequest request, RntbdConstants.Request rntbdRequest)
        {
            ...
            if (request.Properties.TryGetValue(WFConstants.BackendHeaders.EffectivePartitionKey, out object binaryPayload))
            {
        ...
        private static void AddMergeStaticIdIfPresent(DocumentServiceRequest request, RntbdConstants.Request rntbdRequest)
        {
            ...
            if (request.Properties.TryGetValue(WFConstants.BackendHeaders.MergeStaticId, out object binaryPayload))
            {
...
#if COSMOSCLIENT
                else if (binaryPayload is ReadOnlyMemory<byte> binaryRomData)
                {
                    rntbdRequest.mergeStaticId.value.valueBytes = binaryRomData;
                }
#endif
                else
                {
        ...
        private static void AddIndexingDirectiveHeader(RequestNameValueCollection requestHeaders, RntbdConstants.Request rntbdRequest)
        {
            if (!string.IsNullOrEmpty(requestHeaders.IndexingDirective))
            {
                ...
                if (!Enum.TryParse(requestHeaders.IndexingDirective, true, out IndexingDirective directive))
                {
        ...
        private static void AddMigrateCollectionDirectiveHeader(RequestNameValueCollection requestHeaders, RntbdConstants.Request rntbdRequest)
        {
            if (!string.IsNullOrEmpty(requestHeaders.MigrateCollectionDirective))
            {
                ...
                if (!Enum.TryParse(requestHeaders.MigrateCollectionDirective, true, out MigrateCollectionDirective directive))
                {
                    throw new BadRequestException(String.Format(CultureInfo.CurrentUICulture, RMResources.InvalidEnumValue,
        ...
        private static void AddConsistencyLevelHeader(RequestNameValueCollection requestHeaders, RntbdConstants.Request rntbdRequest)
        {
            if (!string.IsNullOrEmpty(requestHeaders.ConsistencyLevel))
            {
                ...
                if (!Enum.TryParse<ConsistencyLevel>(requestHeaders.ConsistencyLevel, true, out ConsistencyLevel consistencyLevel))
                {
                    throw new BadRequestException(String.Format(CultureInfo.CurrentUICulture, RMResources.InvalidEnumValue,
        ...
        private static void AddPopulateIndexMetricsV2(RequestNameValueCollection requestHeaders, RntbdConstants.Request rntbdRequest)
        {
            ...
            {
                rntbdRequest.populateIndexMetricsV2.value.valueByte = requestHeaders.PopulateIndexMetricsV2.
                    Equals(bool.TrueString, StringComparison.OrdinalIgnoreCase)
                    ? (byte)0x01
                    : (byte)0x00;
        ...
        private static void AddOptimisticDirectExecute(RequestNameValueCollection requestHeaders, RntbdConstants.Request rntbdRequest)
        {
            ...
            {
                rntbdRequest.optimisticDirectExecute.value.valueByte = requestHeaders.OptimisticDirectExecute.
                    Equals(bool.TrueString, StringComparison.OrdinalIgnoreCase)
                    ? (byte)0x01
                    : (byte)0x00;
        ...
        private static void AddIsInternalServerlessRequest(RequestNameValueCollection requestHeaders, RntbdConstants.Request rntbdRequest)
        {
            ...
            {
                rntbdRequest.isInternalServerlessRequest.value.valueByte = requestHeaders.IsInternalServerlessRequest.
                    Equals(bool.TrueString, StringComparison.OrdinalIgnoreCase)
                    ? (byte)0x01
                    : (byte)0x00;
...
#if !COSMOSCLIENT
        private static void AddIsServerlessEnabledStorageRefreshRequest(RequestNameValueCollection requestHeaders, RntbdConstants.Request rntbdRequest)
        {
            if (!string.IsNullOrEmpty(requestHeaders.IsServerlessStorageRefreshRequest))
            {
                rntbdRequest.isServerlessStorageRefreshRequest.value.valueByte = (requestHeaders.IsServerlessStorageRefreshRequest.
                    Equals(bool.TrueString, StringComparison.OrdinalIgnoreCase))
                    ? (byte)0x01
                    : (byte)0x00;
                rntbdRequest.isServerlessStorageRefreshRequest.isPresent = true;
            }
        }
#endif
...
#if COSMOSCLIENT
                else if (requestObject is ReadOnlyMemory<byte> endEpkRom)
                {
                    token.value.valueBytes = endEpkRom;
                }
#endif
                else
                {
        ...
        private static void AddUpdateOfferStateToPendingForThroughputSplit(RequestNameValueCollection requestHeaders, RntbdConstants.Request rntbdRequest)
        {
            if (!string.IsNullOrEmpty(requestHeaders.UpdateOfferStateToPendingForThroughputSplit))
            {
                rntbdRequest.updateOfferStateToPendingForThroughputSplit.value.valueByte = requestHeaders.UpdateOfferStateToPendingForThroughputSplit.
                    Equals(bool.TrueString, StringComparison.OrdinalIgnoreCase)
                    ? (byte)0x01
                    : (byte)0x00;
                rntbdRequest.updateOfferStateToPendingForThroughputSplit.isPresent = true;
            }
        }
        ...
        private static void AddUpdateOfferStateToRestorePending(RequestNameValueCollection requestHeaders, RntbdConstants.Request rntbdRequest)
        {
            ...
            {
                rntbdRequest.updateOfferStateToRestorePending.value.valueByte = requestHeaders.UpdateOfferStateToRestorePending.
                    Equals(bool.TrueString, StringComparison.OrdinalIgnoreCase)
                    ? (byte)0x01
                    : (byte)0x00;
        ...
        private static void AddMasterResourcesDeletionPending(RequestNameValueCollection requestHeaders, RntbdConstants.Request rntbdRequest)
        {
            ...
            {
                rntbdRequest.setMasterResourcesDeletionPending.value.valueByte = requestHeaders.SetMasterResourcesDeletionPending.
                    Equals(bool.TrueString, StringComparison.OrdinalIgnoreCase)
                    ? (byte)0x01
                    : (byte)0x00;
        ...
        private static void AddOfferReplaceRURedistribution(RequestNameValueCollection requestHeaders, RntbdConstants.Request rntbdRequest)
        {
            ...
            {
                rntbdRequest.offerReplaceRURedistribution.value.valueByte = requestHeaders.OfferReplaceRURedistribution
                    .Equals(bool.TrueString, StringComparison.OrdinalIgnoreCase)
                    ? (byte)0x01
                    : (byte)0x00;
        ...
        private static void AddHighPriorityForcedBackup(RequestNameValueCollection requestHeaders, RntbdConstants.Request rntbdRequest)
        {
            ...
            {
                rntbdRequest.highPriorityForcedBackup.value.valueByte = requestHeaders.HighPriorityForcedBackup.
                    Equals(bool.TrueString, StringComparison.OrdinalIgnoreCase)
                    ? (byte)0x01
                    : (byte)0x00;
