    ...
    internal class StandByFeedIteratorCore : FeedIteratorInternal
    {
        ...
        public override async Task<ResponseMessage> ReadNextAsync(ITrace trace, CancellationToken cancellationToken = default)
        {
            ...
            do
            {
                ...
                (_, nextKeyRangeId) = await this.compositeContinuationToken.GetCurrentTokenAsync(partitionKeyDefinition, false);
        ...
        internal async Task<Tuple<string, ResponseMessage>> ReadNextInternalAsync(ITrace trace, CancellationToken cancellationToken)
        {
            ...
            PartitionKeyDefinition partitionKeyDefinition = await this.container.GetPartitionKeyDefinitionAsync(cancellationToken);
            (CompositeContinuationToken currentRangeToken, string rangeId) = await this.compositeContinuationToken.GetCurrentTokenAsync(partitionKeyDefinition, false);
            ...
            CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (partitionSplit)
            {
                ...
                await this.compositeContinuationToken.GetCurrentTokenAsync(partitionKeyDefinition: partitionKeyDefinition, forceRefresh: true);
                return true;
            }
