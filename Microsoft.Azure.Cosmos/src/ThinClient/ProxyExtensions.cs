...
namespace Microsoft.Azure.Cosmos.ThinClient
{
    ...
    using PartitionKeyDefinitionVersion = Documents.PartitionKeyDefinitionVersion;

    /// <summary>
    /// Extension methods for the ThinClient Proxy.
    /// Currently used in test - but as the product evolves should consider deleting segments.
    /// </summary>
...
#pragma warning restore CA1034 // Nested types should not be visible
        ...
        /// <summary>
        /// Update the Request message with properties needed for the proxy.
        /// This sets the header RoutedViaProxy on the request *IF* it's one
        /// supported by the proxy, and updates any HTTP headers that can be used
        /// by the custom http client for serialization. If we can get the SDK to
        /// add RequestProperties to the HttpRequest, then we can simply add the DSR
        /// to the Http Properties and skip the rest.
        /// </summary>
        /// <param name="request"></param>
        public static void AddProxyProperties(RequestMessage request)
        {
            ...
            switch (request.OperationType)
            {
                ...
                case OperationType.Read:
                    if (string.IsNullOrEmpty(request.Headers.PartitionKey))
                    {
                        // only requests with PK are routable via proxy.
                        return;
                    }

                    break;
                case OperationType.Query:
                    if (request.Headers[ProxyStartEpk] == null)
                    {
                        // only requests with EPK are routable via proxy.
                        return;
                    }
            ...
            request.Headers.Add(RoutedViaProxy, "1");
            request.Headers.Add(ProxyOperationType, request.OperationType.ToString());
            request.Headers.Add(ProxyResourceType, request.ResourceType.ToString());
        }
        ...
        /// <returns>TODO wololos</returns>
        /// <exception cref="InternalServerErrorException">TODO wololo2</exception>
        ...
        /// TODO wololo
        /// </summary>
        /// <param name="partitionJson"></param>
        ...
        /// <returns>TODO wololo2</returns>
        /// <exception cref="InternalServerErrorException">TODO wololo3</exception>
        public static async Task<HttpResponseMessage> ConvertProxyResponseAsync(HttpResponseMessage responseMessage)
        {
            ...
            BytesDeserializer bytesDeserializer = new BytesDeserializer(metadata, metadata.Length);
            if (!HeadersTransportSerialization.TryParseMandatoryResponseHeaders(ref bytesDeserializer, out bool payloadPresent, out _))
            {
            ...
            bytesDeserializer = new BytesDeserializer(metadata, metadata.Length);
            ...
            HttpResponseMessage response = new HttpResponseMessage(storeResponse.StatusCode);
