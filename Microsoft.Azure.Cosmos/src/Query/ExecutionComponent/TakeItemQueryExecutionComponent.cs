...
namespace Microsoft.Azure.Cosmos.Query.ExecutionComponent
{
    ...
    internal sealed class TakeItemQueryExecutionComponent : ItemQueryExecutionComponentBase
    {
        private readonly TakeEnum takeEnum;
        private int takeCount;

        private TakeItemQueryExecutionComponent(CosmosQueryExecutionComponent source, int takeCount, TakeEnum takeEnum)
            : base(source)
        {
            if (takeCount < 0)
            {
        ...
        public static async Task<TakeItemQueryExecutionComponent> CreateLimitDocumentQueryExecutionComponentAsync(
            int limitCount,
            ...
            Func<string, Task<CosmosQueryExecutionComponent>> createSourceCallback)
        {
            ...
            return new TakeItemQueryExecutionComponent(
                await createSourceCallback(limitContinuationToken.SourceToken),
                limitContinuationToken.Limit,
                TakeEnum.Limit);
        }

        public static async Task<TakeItemQueryExecutionComponent> CreateTopDocumentQueryExecutionComponentAsync(
            int topCount,
            string continuationToken,
            Func<string, Task<CosmosQueryExecutionComponent>> createSourceCallback)
        {
        ...
        private sealed class LimitContinuationToken : TakeContinuationToken
        {
            ...
            {
                if (!TryParse(value, out LimitContinuationToken result))
                {
        ...
        private sealed class TopContinuationToken : TakeContinuationToken
        {
            ...
            {
                if (!TryParse(value, out TopContinuationToken result))
                {
