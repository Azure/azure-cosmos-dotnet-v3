...
namespace Microsoft.Azure.Documents.Rntbd
{
    ...
    internal sealed class LinuxSystemUtilizationReader : SystemUtilizationReaderBase
    {
        ...
        public LinuxSystemUtilizationReader() : this(procStatFilePath: null, procMemoryInfoPath:null)
        {
        }

        internal LinuxSystemUtilizationReader(string procStatFilePath, string procMemoryInfoPath)
        {
            this.procStatFileParser = String.IsNullOrWhiteSpace(procStatFilePath) ? 
                new ProcStatFileParser() : 
                new ProcStatFileParser(procStatFilePath);
            this.procMemInfoFileParser = String.IsNullOrWhiteSpace(procMemoryInfoPath) ? 
                new ProcMemInfoFileParser() : 
                new ProcMemInfoFileParser(procMemoryInfoPath);
        ...
        private class ProcStatFileParser
        {
            ...
            private bool TryReadProcStatFirstLine(ReusableTextReader reusableReader, out string firstLine)
            {
                ...
                {
                    DefaultTrace.TraceError(ioException.Message);
                    firstLine = null;
                    return false;
                }
            }
        ...
        /// <summary>
        /// Read /proc/meminfo file for the memory information. More information can be find here :
        ...
        private class ProcMemInfoFileParser
        {
                ...
                out long totalMemory)
            {
                ...
                try
                {
                    ...
                    {
                        StringParser parser = new StringParser(dataLine, ' ', skipEmpty: true);
                        string prefix = parser.MoveAndExtractNext();
                        if (prefix.Contains("MemFree"))
                        {
                ...
                {
                    DefaultTrace.TraceError(dataException.Message);
                    return false;
                }
            }

            private bool TryReadProcMemInfo(ReusableTextReader reusableReader, 
                out List<string> data)
            {
                try
                {
                    ...
                    using (FileStream fileStream = new FileStream(
                        ...
                        useAsync: false))
                    {
                        data = reusableReader.ReadMultipleLines(fileStream);
                        return true;
                    }
                }
                catch (IOException ioException)
