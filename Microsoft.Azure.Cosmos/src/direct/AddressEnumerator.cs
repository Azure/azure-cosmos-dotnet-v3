...
namespace Microsoft.Azure.Documents
{
    ...
    internal sealed class AddressEnumerator : IAddressEnumerator
    {
                                                                      ...
                                                                      bool replicaAddressValidationEnabled)
        {
            ...
            // Connected/Unknown/UnhealthyPending > Unhealthy.
            return this.ReorderReplicasByHealthStatus(
        ...
        private IEnumerable<TransportAddressUri> ReorderReplicasByHealthStatus(
            ...
            bool replicaAddressValidationEnabled)
        {
            ...
            {
                return this.MoveFailedReplicasToTheEnd(
                    addresses: randomPermutation,
                    failedReplicasPerRequest: failedReplicasPerRequest);
            }
            else
            {
                return this.ReorderAddressesWhenReplicaValidationEnabled(
                    addresses: randomPermutation,
                    failedReplicasPerRequest: failedReplicasPerRequest);
            }
        }
        ...
        /// <returns>The reordered list of <see cref="TransportAddressUri"/>.</returns>
        private IEnumerable<TransportAddressUri> ReorderAddressesWhenReplicaValidationEnabled(
            ...
            HashSet<TransportAddressUri> failedReplicasPerRequest)
        {
            ...
            {
                TransportAddressUri.HealthStatus status = AddressEnumerator.GetEffectiveStatus(
                    addressUri: transportAddressUri,
                    failedEndpoints: failedReplicasPerRequest);

                if (status == TransportAddressUri.HealthStatus.Connected ||
                    status == TransportAddressUri.HealthStatus.Unknown)
                {
                    yield return transportAddressUri;
                }
                else if (status == TransportAddressUri.HealthStatus.UnhealthyPending)
                {
                    pendingReplicas ??= new();
            ...
            HashSet<TransportAddressUri> failedReplicasPerRequest)
        {
            ...
            foreach (TransportAddressUri transportAddressUri in addresses)
            {
                ...
                if (status == TransportAddressUri.HealthStatus.Connected ||
                    status == TransportAddressUri.HealthStatus.Unknown ||
                    status == TransportAddressUri.HealthStatus.UnhealthyPending)
                {
                    yield return transportAddressUri;
                }
                else
        ...
        /// <returns>An instance of <see cref="TransportAddressUri.HealthStatus"/> containing the effective health status of the uri.</returns>
        private static TransportAddressUri.HealthStatus GetEffectiveStatus(
            ...
            HashSet<TransportAddressUri> failedEndpoints)
        {
            ...
            {
                return TransportAddressUri.HealthStatus.Unhealthy;
            }
