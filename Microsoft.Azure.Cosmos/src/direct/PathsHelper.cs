...
namespace Microsoft.Azure.Documents
{
    ...
    internal static class PathsHelper
    {
            ...
            string clientVersion = "")
        {
            ...
            if (PathsHelper.TryParseRootOperation(resourceUrl, out string rootOperationName) &&
                string.Equals(rootOperationName, Paths.Operations_MetadataCheckAccess, StringComparison.OrdinalIgnoreCase))
            {
        ...
        /// <summary>
        /// Tries to parse a root operation name from a resourcURL
        /// 
        /// The format fo a URL will be:
        /// operations/rootOperationName
        /// </summary>
        /// <param name="resourceUrl">Document Service URL to parse.</param>
        /// <param name="rootOperationName">Name of the root operation.</param>
        /// <returns></returns>
        public static bool TryParseRootOperation(
            string resourceUrl,
            out string rootOperationName)
        {
            rootOperationName = null;
            ...
            {
                rootOperationName = segmentOne.GetString();
                return true;
            }
            ...
            bool parseDatabaseAndCollectionNames = false)
        {
            ...
            if (uriSegmentsCount >= 2)
            {
                ...
                else if (firstSegment.Equals(Paths.AzureRbacPathSegment, StringComparison.OrdinalIgnoreCase))
                {
                    if (!ResourceId.TryParse(segments[1], out rid) || !rid.IsAzureRbacId)
                    {
                        isNameBased = true;
                    }
                }
        ...
        private static string GeneratePathForNameBased(ResourceType resourceType, string resourceFullName, bool isFeed, OperationType operationType, bool notRequireValidation = false)
        {
            ...
            else if (resourceType == ResourceType.AuthPolicyElement)
            {
                return Paths.AuthPolicyElementsPathSegment;
            }
            else if (resourceType == ResourceType.AzureRbac)
            {
                return Paths.AzureRbacPathSegment;
            }
            else
        ...
        public static string GeneratePath(ResourceType resourceType, string ownerOrResourceId, bool isFeed, OperationType operationType = default)
        {
            if (isFeed && string.IsNullOrEmpty(ownerOrResourceId) &&
                ...
                resourceType != ResourceType.StorageAuthToken
#endif
                )
            {
            ...
            else if (isFeed && resourceType == ResourceType.AuthPolicyElement)
            {
                return Paths.AuthPolicyElementsPathSegment;
            }
            else if (resourceType == ResourceType.RoleAssignment)
            ...
            {
                return Paths.AuthPolicyElementsPathSegment + "/" + ownerOrResourceId.ToString();
            }
            ...
            else if (isFeed && resourceType == ResourceType.AzureRbac)
            {
                return Paths.AzureRbacPathSegment;
            }
            else if (resourceType == ResourceType.AzureRbac)
            ...
            else if (resourceType == ResourceType.StorageAuthToken)
            {
                return Paths.StorageAuthTokenPathSegment + "/" + ownerOrResourceId;
            }
#endif
        ...
        public static string GenerateRootOperationPath(OperationType operationType)
        {
            switch (operationType)
            {
                ...
                case OperationType.GetAzureRbacAccessCheck:
                    return Paths.OperationsPathSegment + "/" + Paths.Operations_GetAzureRbacAccessCheck;
#endif
        ...
        private static bool IsResourceType(in StringSegment resourcePathSegment)
        {
            ...
            return resourcePathSegment.Equals(Paths.AttachmentsPathSegment, StringComparison.OrdinalIgnoreCase) ||
                   ...
                   resourcePathSegment.Equals(Paths.StorageAuthTokenPathSegment, StringComparison.OrdinalIgnoreCase) ||
        ...
        internal static string[] GetResourcePathArray(ResourceType resourceType)
        {
            ...
            {
                segments.Add(Paths.AuthPolicyElementsPathSegment);
                return segments.ToArray();
            }
            ...
            {
                segments.Add(Paths.AzureRbacPathSegment);
                return segments.ToArray();
            }
        ...
        {
            ResourceId resourceId = null;
            return ResourceId.TryParse(resourceIdString, out resourceId) && resourceId.AuthPolicyElement > 0;
        }
        ...
        internal static bool ValidateAzureRbacId(string resourceIdString)
        {
            ResourceId resourceId = null;
        ...
        internal static bool TryParseOfferResourceLink(string offerResourceLink, out string databaseId, out string collectionId)
        {
            databaseId = null;
            collectionId = null;

            if (string.IsNullOrWhiteSpace(offerResourceLink))
            {
                return false;
            }

            string[] segments = offerResourceLink.Split(RuntimeConstants.Separators.Url, StringSplitOptions.RemoveEmptyEntries);

            if ((segments.Length != 4 && segments.Length != 2))
            {
                return false;
            }
            ...
            if (!string.Equals(segments[2], Paths.CollectionsPathSegment, StringComparison.OrdinalIgnoreCase))
            {
                databaseId = null;
                collectionId = null;
                return false;
