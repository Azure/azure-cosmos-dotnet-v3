...
namespace Microsoft.Azure.Documents.Rntbd
{
    ...
    internal struct SystemUsageLoad
    {
        ...
        public void AppendJsonString(StringBuilder stringBuilder)
        {
            ...
            stringBuilder.Append(this.Timestamp.ToString("O", CultureInfo.InvariantCulture));
            stringBuilder.Append("\",\"cpu\":");
            stringBuilder.Append(this.CpuUsage.HasValue && !float.IsNaN(this.CpuUsage.Value) ? this.CpuUsage.Value.ToString("F3", CultureInfo.InvariantCulture) : "\"no info\"");
            stringBuilder.Append(",\"memory\":");
            stringBuilder.Append(this.MemoryAvailable.HasValue ? this.MemoryAvailable.Value.ToString("F3", CultureInfo.InvariantCulture) : "\"no info\"");
