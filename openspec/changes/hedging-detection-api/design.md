...
- Expose the responding region name when hedging was active
...
**Choice**: Add `IsHedged()` and `GetRespondingRegion()` as virtual methods on `CosmosDiagnostics`.
...
### Decision 2: Boolean flag + string field on `CosmosTraceDiagnostics` (not trace Data dictionary lookup)

**Choice**: Add `internal bool isHedged` and `internal string respondingRegion` fields directly to `CosmosTraceDiagnostics`. The hedging strategy sets these fields at the same call site where it already calls `AddOrUpdateDatum`.

**Rationale**: Accessing `ITrace.Data` requires calling `SetWalkingStateRecursively()` first (which locks and walks the entire trace tree). A direct field on the diagnostics object is a true O(1) read with zero side effects — no locking, no tree walking, no dictionary lookup.
...
**Choice**: `IsHedged()` returns `false` and `GetRespondingRegion()` returns `null` by default.
...
### Decision 4: Method names `IsHedged()` and `GetRespondingRegion()`

**Choice**: `bool IsHedged()` — matches the semantic question "was this request hedged?" `string GetRespondingRegion()` — returns the region name that served the winning response.

**Rationale**: Consistent with existing naming (`GetClientElapsedTime`, `GetContactedRegions`, `GetFailedRequestCount`). Using methods rather than properties matches the established convention in `CosmosDiagnostics` where all accessors are methods.
...
**[Risk] Flag not set on non-hedging code paths** → By design. The flag defaults to `false`/`null`, and only the hedging strategy sets it. If new availability strategies are added, they would need to set the flag independently. Mitigation: document this contract in the internal field comments.

**[Trade-off] Two separate methods vs. a composite return type** → Two simple methods are easier to use in conditional checks (`if (diagnostics.IsHedged())`) than unpacking a composite object. If richer hedging metadata is needed later, a `GetHedgingInfo()` method returning a structured type can be added without breaking these methods.

**[Trade-off] `GetRespondingRegion()` returns null for non-hedged requests** → This makes it clearly tied to hedging. The existing `GetContactedRegions()` already covers the general "which region(s) were used" question.
