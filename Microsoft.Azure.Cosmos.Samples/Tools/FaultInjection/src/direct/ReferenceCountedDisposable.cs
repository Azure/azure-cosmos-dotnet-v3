// Based on https://github.com/dotnet/roslyn/blob/38e1b12b27bbe15ed5e0da3f90ea73e025ff5c7b/src/Workspaces/SharedUtilitiesAndExtensions/Compiler/Core/Utilities/ReferenceCountedDisposable.cs,
// but with WeakReference functionality removed.
...
#nullable enable
        ...
        where T : class, IDisposable
    {
        ...
        /// counted reference to the same target object. Accesses to <see cref="BoxedReferenceCount.Value"/>
        ...
        {
#if DEBUG_REF_COUNTS
            this._objectId = Interlocked.Increment(ref s_nextObjectId);
#endif
            ...
            this._boxedReferenceCount = referenceCount;
        }
        ...
        public T Target => this._instance ?? throw new ObjectDisposedException(nameof(ReferenceCountedDisposable<T>));
        ...
        {
            lock (this._boxedReferenceCount)
            {
                if (this._boxedReferenceCount.Value == 0)
                {
                ...
                checked
                {
                    this._boxedReferenceCount.Value++;
                }

                // Must return a new instance, in order for the Dispose operation on each individual instance to
                // be idempotent.
                return new ReferenceCountedDisposable<T>(this._instance, this._boxedReferenceCount);
            }
        }
        ...
        public void Dispose()
        {
            ...
            lock (this._boxedReferenceCount)
            {
                ...
                if (this._boxedReferenceCount.Value == 0)
                {
                    instanceToDispose = this._instance;
                }
