...
namespace Microsoft.Azure.Cosmos.Encryption
{
    ...
    internal sealed class AapContainer : Container
    {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                throw new NotSupportedException($"{nameof(partitionKey)} cannot be null for operations using {nameof(AapContainer)}.");
            }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            using (diagnosticsContext.CreateScope("CreateItemStream"))
            {
                    ...
                    encryptionItemRequestOptions.EncryptionOptions != null)
                {
                    ...
                    streamPayload = await AapEncryptionProcessor.EncryptAsync(
                        streamPayload,
                        this.Encryptor,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (partitionKey == null)
            {
                throw new ArgumentNullException($"{nameof(partitionKey)} cannot be null for operations using {nameof(AapContainer)}.");
            }
        ...
        public override TransactionalBatch CreateTransactionalBatch(
            PartitionKey partitionKey)
        {
            return new AapTransactionalBatch(
            ...
            QueryRequestOptions requestOptions = null)
        {
            return new AapFeedIterator<T>(
                (AapFeedIterator)this.GetItemQueryStreamIterator(
                    queryDefinition,
                    continuationToken,
                    requestOptions),
        ...
        {
            return new AapFeedIterator<T>(
               (AapFeedIterator)this.GetChangeFeedStreamIterator(
                   feedRange,
                   changeFeedRequestOptions),
               this.ResponseFactory);
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            using (JsonDocument doc = JsonDocument.Parse(input))
            {
                try
                {
                    await AapEncryptionProcessor.DecryptAndWriteAsync(doc.RootElement, this.Encryptor, writer, encryptionProperties: encryptionProperties, cancellationToken: cancellationToken);
                }
