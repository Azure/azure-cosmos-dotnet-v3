...
Kept as one-liners because these IDs are referenced throughout the document. Full reasoning for each is in §13.
...
| Date | Change |
| --- | --- |
| 2026-09-23 | **Corrected the goal framing and audited §10.** The header said “remove the `Microsoft.Azure.Cosmos.Direct` dependency entirely — the SDK compiles the Direct sources rather than referencing the shipped assembly”, written when option A was the working assumption; read literally it says `Direct.dll` goes away, which is the opposite of D-0's resolution. Rewrote it to separate **removing the package dependency** from **retiring the assembly**, with an explicit callout that option C keeps `Direct.dll` shipping and that folding the types into `Client.dll` was the rejected option A. Fixed §1 in-scope items 2 (still cited the abandoned internal feed) and 3 (now resolved by D-0), and the §8 phase-2 gate row. Audited every §10 row: **closed R-10** (option A only — reopens only if R-8 fails); **expanded R-5** — with Direct source-compiled and the native from `QueryPlanInterop`, `Microsoft.HybridRow` is now the *last* binary dependency, raised as **O-8** (scope question, blocks nothing, but phase 5 should not claim “compile from source” while it is unanswered). Added a phase-3 task to re-express `UseDirectSource` for the split-project shape, since the `<Compile Remove>` form stops working once the sources live in `Direct.csproj`. |
| 2026-09-23 | **R-1 re-baseline procedure written** (§8 phase 1a), targeting Direct **3.44.1** from msdata `sdkReleases/direct/EN20260409-3.44.1`. Measured the topology rather than estimating it: `merge-base` is `f5effced7` (2022-10-26), `origin/msdata/direct` is 61 commits ahead of that base and **1072 behind `main`**. That settles the approach — **re-import, do not merge**: a three-year-old base against 1072 commits of unrelated `main` movement conflicts in files unrelated to Direct, and `src/direct/**` is a mirror with no v3-side authorship worth preserving. Six steps with an explicit acceptance gate (surface vs. `contracts/DirectSDKAPI.json`, `Version=3.44.1.0`, `PublicKeyToken=31bf3856ad364e35`, 48 IVT grants, full suite under `UseDirectSource=true`) and a standing rule that a gate failure means the *import* is wrong — never patch the source to make it pass. Also recorded that C-3's `git mv` constraint governs 1b only, and that three of the four msdata-ish refs in this clone (350 / 363 / 373 files) are stale traps. **Step 0 is the remaining blocker:** this clone has `origin` as its only remote, so msdata source access must be arranged before phase 1 starts. |
| 2026-09-23 | **O-7 closed — the reference Direct version is `3.44.1`**; **O-5 cadence answered — `QueryPlanInterop` ships every ~2 months.** Verified 3.44.1 on nuget.org (it is absent from the default search index without `semVerLevel=2.0.0`): `Version=3.44.1.0`, `PublicKeyToken=31bf3856ad364e35`, **48** `InternalsVisibleTo` grants, all five natives present. Repointed every reference from 3.44.0 to 3.44.1 — the R-18 `AssemblyVersion` pin (now `3.44.1.0`, verified rather than assumed), the R-17 friend-assembly cross-check, the R-8 strong-name check, §9.E Gate 1's `contracts/DirectSDKAPI.json` capture, phase 1a and §11. Updated P-12: **both** sides move in the re-baseline, since `msdata/direct` is at 3.43.2 and `main`'s `<DirectVersion>` is 3.44.0. The 2-month cadence fully downgrades R-15 and turns F-2 into a recurring absorption process (~6 bumps/year) rather than a one-off pin. O-5's remaining half — which ServiceInterop build `QueryPlanInterop` 1.0.2 corresponds to — is now sharper: 3.44.1's `ServiceInterop.dll` is 9.14 MB against QueryPlanInterop's 7.40 MB, and the two artifacts have independent cadences and version schemes. **No decision blocks phase 1 any more**; the R-1 procedure is the only thing left gating it. |
| 2026-09-23 | **D-3 revised a second time: adopt the public `Microsoft.Azure.Cosmos.QueryPlanInterop.Windows` package.** Supersedes both “commit the binaries” and the internal-`CosmosDB-Internal`-feed draft. The package is on **public nuget.org**, owned by `azure-sdk`, with `projectUrl` pointing at this repo. Measured against Direct 3.44.0's natives: **1 file instead of 5**, 7.40 MB vs 9.25 MB, and its import table shows **only OS DLLs** — it is statically linked against the CRT, so `msvcp140`, `vcruntime140`, `vcruntime140_1` and `Cosmos.CRTCompat` are no longer needed at all (this closes the standing VC++-redistributable open item). It carries the identical C++ types and exports exactly the six externs `ServiceInteropWrapper.cs` declares; nothing in the repo uses the dropped `Crc32/64ComputeInterop` or JNI exports. The only mismatch is the filename, solved by a `ContentWithTargetPath` `TargetPath` rename — **zero source edits**, so the wrapper stays byte-identical to msdata and F-4's guards are untouched. Replaced the pipeline acquisition step with a plain `PackageReference`. **Closed R-16**; narrowed O-5 to provenance/cadence. New customer-observable consequence: the nupkg stops shipping the four VC++/CRT DLLs — changelog entry required. Added a functional-equivalence gate to phase 2, since 7.40 vs 8.47 MB is a different build. |
| 2026-09-23 | **Consistency sweep after D-0/D-3/O-1.** Raised **O-7**: the imported sources are Direct **3.43.2**-era while the `AssemblyVersion` pin, contract baseline, `InternalsVisibleTo` extraction and strong-name check are all against **3.44.0** — §9.E Gate 1 would fail on day one. Must be settled inside the R-1 procedure. Flagged that the `UseDirectSource` escape hatch (`<Compile Remove>`) only expresses anything while the Direct sources are compiled by `Microsoft.Azure.Cosmos.csproj`; from phase 3 under option C it must toggle `PackageReference` ↔ `ProjectReference` instead. Fixed three statements left contradicting the resolved decisions: D-8's `Direct/README.md` instruction (“mirrored from msdata, must not be hand-edited” — inverted by O-1), §11's ApiCompat-against-3.43.2 item (moot under option C), and §8's “behind one blocking decision gate”. Corrected the gate status: the three original decisions are closed, but O-6 and O-7 are open and block phases 3 and 1 respectively. |
| 2026-09-23 | **O-1 resolved: v3 `main` is upstream, msdata is downstream.** This was the last decision in the §8 gate, which is now **closed**. Inverted the dependent machinery: §9.E **Gate 2 is authoritative** (Gate 3 becomes informational — it measures msdata's lag rather than gating v3); §9.A-4 T0 changes now **originate here** with msdata sign-off instead of “upstream only”; §9.A-7's merge-based inbound sync is scoped to the phases 1–4 catch-up window and retired in phase 5; D-7's tooling inverts from import to **export**, and its manifest should be built bidirectional so the flip is configuration rather than a second tool; `src/Direct/` is hand-editable by design, so `Direct/README.md` carries house rules rather than a “do not hand-edit” notice (§5). Narrowed O-6 — with v3 upstream, retiring `ProjectRef` after the export path lands is the likely answer, but phase-3 behaviour still needs the msdata owners. Rewrote §12: no decisions remain, only inputs and the R-1 re-baseline procedure, which is now the largest unwritten piece. |
| 2026-09-23 | **D-0 resolved: option C** — `Microsoft.Azure.Documents.*` keeps shipping from `Microsoft.Azure.Cosmos.Direct.dll`, built from this repo. Added the framing that was missing from every prior draft: the v3 nupkg **already** packs `Direct.dll` ([csproj L167](Microsoft.Azure.Cosmos/src/Microsoft.Azure.Cosmos.csproj#L167)), so option C ships nothing new and option A is the change. R-8 evidence is now favourable — shipped identity is `Version=3.44.0.0, PublicKeyToken=31bf3856ad364e35`, the standard Microsoft shared-library token that `35MSSharedLib1024.snk` produces — but the formal check still runs first. Two prerequisites discovered and added as **R-17** and **R-18**: the `InternalsVisibleTo` graph is absent from all 371 imported files yet the shipped assembly carries **48** friend grants (mostly service-side — `CosmosDB.Runtime`, `Compute.*`, `Sql.Service`, `Mongo.Service*`, `Cassandra.*`, `DataTransfer.*`, `Portal.Services.Backend`), and `AssemblyVersion` must be pinned to `3.44.0.0` rather than inheriting `ClientOfficialVersion`. Closed O-3 and O-4. Flagged `.nuspec` dependency leakage and two-assembly signing as verify-items. Updated §8 phase 0 (O-1 is now the only open decision), phase 1a, §11 and §12. |
| 2026-09-23 | **D-3 revised: restore the natives from the internal `CosmosDB-Internal` feed instead of committing them.** `ServiceInterop` is published there as its own package, which invalidated the single premise behind rejecting option A on 2026-09-22 ("no such package exists"). Two facts settle it: `ProjectRef=True` already drops *both* the Direct `PackageReference` and the whole native pack/copy `ItemGroup`, so msdata supplies its own natives and the **public v3 build is the sole consumer**; and this repo's PR validation runs on the internal `OneES` pool, so **every CI gate can authenticate** — an earlier draft assumed public CI and overstated the cost. Acquisition is a **pipeline step plus an MSBuild property at build time**, never a `PackageReference`, keeping the restore graph and the (absent) `NuGet.config` untouched; build time rather than pack time because `DirectContractTests` P/Invokes the native, not just checks a flag. Closes R-14 and the Git LFS retrofit, drops `native-assets.json` and the SHA/Authenticode gates, and downgrades R-15 from an engine freeze to a version bump. Adds R-16 (external contributors lose the interop path locally) and narrows O-5 to cadence/ownership plus the nuget.org parity question. Rewrote C-7, §5, D-3, phase 2, §9.F F-2, §11 and §12; option C retained as the documented fallback. |
| 2026-09-22 | **D-3 resolved: commit the native binaries.** *(Superseded 2026-09-23.)* The `Microsoft.Azure.Cosmos.Direct` package is being discontinued, which was believed to eliminate the natives-only-package (A) and restore-only (B) options. Rewrote D-3 with the supporting measurements — ten releases each shipping a byte-different native, `FileVersion` frozen at `2.14.0.0` across all of them, and all five assets individually Authenticode-signed — plus six required conditions. Added C-7, R-14, R-15, O-5; closed O-2. Reworked §9.F. **Corrected F-4** — it previously said to strip the wrapper's `#if COSMOSCLIENT` guards; they are load-bearing and select the only DLL name that ships. Updated §1, §2.4, §5, D-1, phases 0/2/5, §11 and §12 to match. |
