...
- [5976](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/5976) Barrier Early Yield: Adds `CosmosClientOptions.EnableBarrierEarlyYieldOn429` (default true) to enable barrier early yield on 429 optimization. Updates Direct Package version to `3.44.0` which includes ConsistencyWriter and QuorumReader yielding early with substatus 21013 (`Server_WriteBarrierThrottled`) when all replicas return 429 during barrier requests, allowing `ResourceThrottleRetryPolicy` to handle retries instead of spinning until timeout.
