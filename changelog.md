...
- [5989](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/5989) Distributed Transactions (preview): Fixes a bodyless `429`/`3200` (RUBudgetExceeded) response on a distributed-transaction commit or read being surfaced to the caller without any retry. The empty response body was misdetected as a semantic per-operation result and deferred to the transaction's outer retry loop, which could not act on it, so the request was never re-sent. Such a throttled response is now retried honoring the server's `x-ms-retry-after-ms` header and the customer-configured rate-limit retry options (`CosmosClientOptions.MaxRetryAttemptsOnRateLimitedRequests` and `MaxRetryWaitTimeOnRateLimitedRequests`, defaults 9 attempts / 30 seconds cumulative), so a coordinator returning large retry-after values cannot stall a commit indefinitely.
- [6017](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/6017) Cross Region Hedging: Hardens the cross-region hedging strategy so that a losing hedge arm cancelled after another region has already produced the winning response can never surface a non-cancellation exception to the caller — for example an `ArgumentNullException` ("Value cannot be null. (Parameter 'request')") from a request cancelled before dispatch, or an `ObjectDisposedException` from a raced disposed clone — nor crash the process as an unobserved `TaskScheduler.UnobservedTaskException`. Any such post-cancellation exception from a losing hedge is now normalized to `CosmosOperationCanceledException` and the abandoned hedge tasks are observed, so the winning region's response is always returned unaffected. This is the defense-in-depth companion to the source-level guard in [#6016](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/6016) (issue [#6014](https://github.com/Azure/azure-cosmos-dotnet-v3/issues/6014)), and is most relevant for accounts where hedging is implicitly enabled by Per-Partition Automatic Failover (PPAF), where no explicit customer opt-in is required to exercise the path.
