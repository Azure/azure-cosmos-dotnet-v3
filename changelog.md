...
- [5927](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/5927) ThinClient: Fixes mid-flight fallback to gateway when the service stops advertising thin-client endpoints. Previously the SDK kept routing to stale thin-client URIs and required a client restart to recover; now `LocationCache.HasThinClientReadLocations` and `LocationCache.HasThinClientWriteLocations` are re-evaluated on every account refresh and per-request dispatch gates fall back to the regular gateway path on the very next request. The two directions are tracked independently so that partial withdrawal (service drops only thin-client reads or only writes) also flips back to gateway for the affected direction.
