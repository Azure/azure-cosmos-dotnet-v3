...
- [5958](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/) DistributedTransaction (preview): Adds public `SessionToken` property on `DistributedTransactionOperationResult`, allowing callers to pass per-operation session tokens back via `DistributedTransactionRequestOptions.SessionToken` to enforce read-your-writes session consistency.

#### Breaking Changes

#### Bugs Fixed

- [5958](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/5958) DistributedTransaction (preview): Fixes subsequent Session-consistency reads possibly failing with `ReadSessionNotAvailable` (404/1002) after a partition split. A distributed-transaction commit is an HTTP 207 multi-status response, so a per-operation partition-not-found never reaches the internal session-mismatch retry policy that point operations rely on to auto-recover. The commit now mirrors the point-operation capture path: when the server serves an operation from a different partition key range than the client resolved, the partition routing cache is force-refreshed (once per moved range) so later Session reads on the affected key resolve the current range.
