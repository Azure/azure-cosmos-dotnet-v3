...
- [6079](https://github.com/Azure/azure-cosmos-dotnet-v3/pulls/6079) ChangeFeedProcessor: Fixes a `NullReferenceException` on partition split/merge when a lease's `FeedRange` was missing (for example, a lease rehydrated from previously persisted in-memory lease state). The affected range would stall permanently until the host process was restarted. The in-memory lease manager now backfills the missing `FeedRange` when a lease is acquired, and the split/merge handler falls back to resolving it from the routing map cache as a safety net, so the split/merge completes normally instead of throwing.
