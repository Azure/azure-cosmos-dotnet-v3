...
- [5997](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/5997) Diagnostics: Adds a per-node upper bound on the number of child traces retained under a single diagnostics node, so an operation that retries the same work without limit at one point in the tree (for example a cross-partition query whose background prefetch keeps retrying transport-generated `410`s against an unreachable region) can no longer grow that node without bound. Once the limit is reached, additional child traces are omitted, the node reports how many were dropped via a `Truncated Child Trace Count` entry, and the top-level `Summary` is marked with `PartialResults: true`, in which case the walk-computed `Summary` histogram counts become lower bounds (the imperatively tracked aggregate failure/region/hedging counts remain accurate). The bound is on per-node breadth rather than total tree size, and is deliberately set well above the widest realistic fan-out (a healthy cross-partition query attaches one child per physical partition to a single node), so it acts purely as a runaway-growth guardrail and no normal operation loses diagnostics fidelity.
