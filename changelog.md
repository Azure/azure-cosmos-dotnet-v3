...
- [6076](https://github.com/Azure/azure-cosmos-dotnet-v3/pulls/6076) ChangeFeedProcessor: Fixes a `NullReferenceException` thrown from `PartitionSynchronizerCore.HandlePartitionGoneAsync` on a partition split/merge notification, when the lease's `FeedRange` was not populated (for example, a PKRange-based lease rehydrated from previously persisted in-memory lease state, since `FeedRange` is serialized with `NullValueHandling.Ignore`). The exception was thrown before any child lease was created and before the parent lease was deleted, so the affected range would stall permanently until the host process was restarted. The SDK now resolves the lease's range from the routing map cache using its `PartitionKeyRangeId` when `FeedRange` is missing, so the split/merge completes normally instead of throwing.
