...
- [5958](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/5958) DistributedTransaction (preview): Fixes session-token application to match point operations. A write sub-operation on a single-master account is no longer assigned a session token (matching the gateway store model's write gate), while reads and writes on multi-master accounts continue to send one. A caller-supplied `DistributedTransactionRequestOptions.SessionToken` is always honored.
