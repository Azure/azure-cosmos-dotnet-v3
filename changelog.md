...
- [XXXX](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/XXXX) ThinClient: Fixes a regression where a client that started with an account not yet advertising thin-client locations could not dynamically transition to thin-client routing after a later account refresh began advertising them, even though the reverse direction (thin-client → gateway) worked correctly. The construction of `ThinClientStoreModel` and the wiring of the connectivity probe are no longer gated on the initial `ThinClientWritableLocations` count, restoring the bidirectional dynamic-enable contract introduced in [#5927](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/5927): the same client instance now switches between thin-client and gateway V1 on each account refresh with no client restart required. Also gates `ThinClientStoreModel.ShouldResolvePartitionKeyRange` on the live `HasThinClientRead/WriteLocations` flags so Gateway-mode clients whose account never advertises thin-client endpoints do not incur eager PKR resolution that plain `GatewayStoreModel` (PPAF/PPCB-gated) would not have done.
