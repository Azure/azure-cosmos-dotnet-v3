...
- Hierarchical Partition Key: Adds automatic retry when the backend returns `BadRequest` with sub-status `1038` indicating the item's `id` must be appended as the last partition key component. On this response the SDK marks the container as requiring `id` as the last partition key path and transparently retries the operation with `id` appended to the partition key. This applies to point operations, `ReadManyItems`, and bulk operations so they continue to succeed during an "add id to partition key" migration; a transactional batch (whose operations share a single partition key but span different `id` values) cannot append the `id` and fails with an `ArgumentException`.
