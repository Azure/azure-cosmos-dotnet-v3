...
- [5917](https://github.com/Azure/azure-cosmos-dotnet-v3/issues/5917) Metadata Hedging: Adds internal cross-region hedging for the Collection and PartitionKeyRange metadata caches, reducing operation latency tails when the primary region is slow on these metadata reads. Hedging applies to both first-population (cold-start) and steady-state refresh reads of those caches and is restricted by request type (`Collection` `Read`, `PartitionKeyRange` `ReadFeed` first page); a hedge only fires when the primary exceeds a fixed latency threshold, and a per-client concurrency budget bounds the secondary-region fan-out. There is no public configuration API in Phase 1: in the preview package metadata hedging is enabled by default, while in the GA package hedging follows the account's PPAF (Per-Partition Automatic Failover) state — active for PPAF-enabled multi-region accounts — and can additionally be force-enabled (for non-PPAF accounts) or suppressed through the `AZURE_COSMOS_METADATA_HEDGING_ENABLED` environment variable (which overrides the default in both packages). It is observable through the new `Azure.Cosmos.Client.MetadataHedging` Meter and the `Metadata Hedge Context` diagnostics block. See [docs/PPAF_Metadata_Hedging_ColdStart_Design.md](https://github.com/Azure/azure-cosmos-dotnet-v3/blob/main/docs/PPAF_Metadata_Hedging_ColdStart_Design.md).
