...
- [5913](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/5913) RetryPolicies: Fixes operation-level `CancellationToken` being ignored across multiple retry/backoff and metadata-refresh paths — `ClientRetryPolicy` (cross-region retries on `Gone (410)` / `LeaseNotFound (1022)` and other transient backend status codes), `ResourceThrottleRetryPolicy` (`429` throttling backoffs up to 60s by default), `WebExceptionRetryPolicy` (exponential backoff on retriable HTTP/socket exceptions), `BulkExecutionRetryPolicy`, the gateway HTTP retry loop in `CosmosHttpClientCore`, and `PartitionKeyRangeCache` lookups used by query pagination (cross-partition fan-out, split/merge handling, and PKRange cache refresh between pages). Operations now surface `OperationCanceledException` within the caller's token deadline instead of running additional retries, holding inter-attempt delays, or blocking on partition-key-range cache refreshes past it.
