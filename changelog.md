...
- Read Consistency Strategy: Fixes reads using `ReadConsistencyStrategy.LastCommittedSingleWriteRegion` hanging indefinitely on single write region accounts when the client's application region is a read replica rather than the hub. The SDK correctly recorded a partition level override pointing at the hub region after each `403/WriteForbidden`, but the dispatch path did not consult that override, so every retry was re-routed to the original non-hub region. Because the endpoint actually used never changed, the retry loop could not converge, had no backoff, and only ended when the caller's cancellation token fired. Dispatch now consults the same hub region routing signal used when marking the endpoint unavailable, so retries reach the hub region and the operation completes. Configuring a cross region hedging availability strategy previously hid the problem, because each hedge arm is pinned to a single region and the arm targeting the hub succeeded while the primary arm continued to spin.
