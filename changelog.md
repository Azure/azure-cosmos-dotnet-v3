...
- [6085](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/6085) ThinClient: Fixes the same client instance not being able to dynamically switch to thin-client routing when the account starts without thin-client endpoints and later begins advertising them. The routing decision is now made per request against live topology flags, so the client transitions in both directions on account refresh without a restart, and Gateway-mode clients whose account never advertises thin-client endpoints retain the plain gateway overhead profile. Also fixes a resource leak where, when thin-client (or Direct) mode is active, the separate `GatewayStoreModel` instance was never disposed with the client; `CosmosClient`/`DocumentClient` disposal now disposes both store models. Also fixes the transport diagnostics label so a request that is served by the gateway path (because thin-client is unavailable for it) is reported under `GatewayStoreModel` instead of `ThinClientStoreModel`, and vice versa, matching the store model that actually serves the request. The thin-client connectivity probe now also accepts an HTTP 401 (in addition to 200) as proof the regional proxy endpoint is reachable, since the probe is sent without an authorization header; per-request authorization is unaffected. Finally, the `F4` thin-client user-agent feature flag is now dynamic: it is emitted only while the account is actually advertising thin-client endpoints and is added or removed on account refresh as availability changes, so the user agent reflects whether the client is currently routing through the thin-client proxy. As part of this, a latent user-agent bug is fixed where re-emitting a feature flag could duplicate it (for example `F4|F4`), fail to remove it, or leave a hexadecimal flag (such as `FC`) uncleared — user-agent feature flags are now composed without corrupting a configured application name.
...
- [5955](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/5955) Web Exception Retry: Fixes the connection-failure retry budget so it stops retrying at the intended 30-second limit. The remaining-time calculation previously used the seconds component of the elapsed `TimeSpan` (which wraps every 60 seconds) instead of the total elapsed time, so once cumulative retries crossed the one-minute mark the policy could keep retrying well beyond its budget. Retries now honor the full elapsed duration.
...
- [6013](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/6013) AAD Token Revocation: Gates the AAD token revocation / CAE retry feature behind the `AZURE_COSMOS_AAD_TOKEN_REVOCATION_ENABLED` environment variable (default OFF). When unset, the SDK preserves the exact pre-feature AAD behavior; setting the variable to `true` opts in to the revocation retry and `cp1` CAE capability.
...
| ChangeFeedProcessor in `ChangeFeedMode.AllVersionsAndDeletes` fails to start with HTTP 400 on cold start (no persisted lease continuation token). | New AVAD ChangeFeedProcessor deployments cannot start. Existing processors that already have a persisted lease continuation token are not affected; LatestVersion-mode processors are not affected. | Upgrade to [3.60.0](#3.60.0) (or later). | [#5846](https://github.com/Azure/azure-cosmos-dotnet-v3/issues/5846) | 
