...
- [5968](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/5968) (Preview) Query: Adds Full-Text Policy standard package and analyzer configuration. Enables specifying `package` ("legacy" or "standard"), `defaultSpec` (language, tokenizer, filters, stopWordListKind, addStopWords, removeStopWords), and per-path overrides on the full-text policy. This allows fine-grained control over text analysis including tokenizer selection, filter pipelines, and custom stop word lists.
