...
- [5983](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/5983) Direct: Fixes silent truncation of `IdleTcpConnectionTimeout`, mirroring the `OpenTcpConnectionTimeout` fix in [#5873](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/5873). Sub-second values in [0, 1s) are now explicitly normalized to 0 (no idle timeout) and fractional values ≥ 1 second are rounded up to the nearest whole second (for example, 2.3s becomes 3s) instead of being truncated down. Negative values emit a warning trace (Direct mode only) but are left unchanged for backward compatibility. Resolves [#5874](https://github.com/Azure/azure-cosmos-dotnet-v3/issues/5874).
