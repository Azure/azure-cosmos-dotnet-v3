...
- [5958](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/XXXX) Distributed Transactions (preview): Fixes session consistency for committed transactions by automatically resolving and attaching the correct per-partition session token to each sub-operation, so reads within and after a Session-consistency transaction observe the transaction's own writes (read-your-writes). Each operation's partition is resolved so the exact partition-local token is applied (never the compound collection-wide token), matching the point-operation write-gate (a write on a single-master account carries no auto-resolved token, while reads and multi-master writes do). If routing metadata cannot be resolved while the client already holds causal progress for the collection, the commit now fails instead of silently degrading to a tokenless request, preserving session guarantees during transient metadata failures. Callers may still pass a token explicitly via `DistributedTransactionRequestOptions.SessionToken`, which is always honored.
- [6066](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/6066) Distributed Transactions (preview): Fixes session consistency for committed transactions by automatically resolving and attaching the correct per-partition session token to each sub-operation, so reads within and after a Session-consistency transaction observe the transaction's own writes (read-your-writes). Each operation's partition is resolved so the exact partition-local token is applied (never the compound collection-wide token), matching the point-operation write-gate (a write on a single-master account carries no auto-resolved token, while reads and multi-master writes do). If routing metadata cannot be resolved while the client already holds causal progress for the collection, the commit now fails instead of silently degrading to a tokenless request, preserving session guarantees during transient metadata failures. Callers may still pass a token explicitly via `DistributedTransactionRequestOptions.SessionToken`, which is always honored.
...
- [6066](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/6066) Distributed Transactions (preview): Fixes session consistency for committed transactions by automatically resolving and attaching the correct per-partition session token to each sub-operation, so reads within and after a Session-consistency transaction observe the transaction's own writes (read-your-writes). A caller-supplied token via `DistributedTransactionRequestOptions.SessionToken` is still always honored.
