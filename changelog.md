...
- [5997](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/5997) Diagnostics: Adds a per-node upper bound on the number of child traces retained under a single diagnostics node so a long-running operation that retries the same work many times at one point in the tree (for example a cross-partition query whose background prefetch keeps retrying transport-generated `410`s against an unreachable region, observed producing a 159 MB diagnostics string from a single node) can no longer grow that node without limit. Once the limit is reached, additional child traces are omitted, the node reports how many were dropped via a `Truncated Child Trace Count` entry, and the top-level `Summary` is marked with `PartialResults: true`. The bound is on per-node breadth rather than total tree size. Typical operations are unaffected; only pathological, effectively unbounded per-node fan-out is truncated, in which case the walk-computed `Summary` histogram counts become lower bounds (the imperatively tracked aggregate failure/region/hedging counts remain accurate).
