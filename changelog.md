...
- [#6067](https://github.com/Azure/azure-cosmos-dotnet-v3/issues/6067) Diagnostics: Fixes an unobserved `TaskScheduler.UnobservedTaskException` (`ArgumentException` "Identifier {0} does not exist. Please call start before calling end.") that could surface from a background address cache refresh, introduced in 3.62.0. The background refresh is fire-and-forget and its failure was only observed while verbose tracing was enabled, so at the production default tracing level the faulted task was never observed and was raised at finalization instead. The failure is now always observed. Additionally, the underlying address resolution bookkeeping race is fixed: a background refresh that outlived the attempt which started it could try to complete its resolution against a replaced diagnostics instance, which no longer produces an exception. Requests were never failed by this; the impact was limited to the unobserved exception surface.
