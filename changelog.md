...
- [6000](https://github.com/Azure/azure-cosmos-dotnet-v3/issues/6000) Gateway: Adds `AZURE_COSMOS_HTTP_POOLED_CONNECTION_LIFETIME_IN_MILLISECONDS` and `AZURE_COSMOS_HTTP_POOLED_CONNECTION_IDLE_TIMEOUT_IN_MILLISECONDS` environment variables to tune the SDK's default `SocketsHttpHandler` connection-pool timeouts in Gateway mode. Servers that recycle keep-alive connections aggressively (for example the vNext Linux Cosmos emulator) can leave a pooled HTTP/1.1 connection that the server has already closed; a non-idempotent write dispatched onto it then blocks for the full request timeout instead of failing fast. Lowering the pooled-connection lifetime and/or idle timeout shrinks the reuse window, so callers no longer have to replace the entire `HttpClientFactory` to work around it. Both default to the previous behavior when unset (randomized 5–5.5 min lifetime; 1 min idle timeout).
- [5977](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/5977) Diagnostics: Fixes missing CPU/memory ("System Info") in the diagnostics of failed operations. The system usage was only attached after a successful inner send, so operations that ended in an exception (for example request timeouts, cancellations, or exhausted retries) - exactly when client CPU/memory is most useful - had no CPU/memory data in their diagnostics. The capture now runs on both the success and the failure path while remaining best-effort (it never impacts the request). The system usage was only attached after a successful inner send, so operations that ended in an exception (for example request timeouts, cancellations, or exhausted retries) - exactly when client CPU/memory is most useful - had no CPU/memory data in their diagnostics. The capture now runs on both the success and the failure path while remaining best-effort (it never impacts the request).
