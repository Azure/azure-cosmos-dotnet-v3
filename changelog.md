...
- [5976](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/5976) Barrier Early Yield: Updates Direct package to `3.44.0` which includes ConsistencyWriter and QuorumReader yielding early with substatus 21013 (`Server_WriteBarrierThrottled`) when all replicas return 429 during barrier requests, allowing `ResourceThrottleRetryPolicy` to handle retries instead of spinning until timeout. Note: this Direct version bump introduces 408 responses with barrier-specific substatus codes (e.g., 21013) for barrier failures that previously surfaced as plain timeouts. Adds internal opt-out flag and guards `ClientRetryPolicy` from marking the endpoint unavailable on the synthetic 408/21013.
