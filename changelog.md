...
- [5549](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/5549) AAD Token Revocation: Gates the AAD token revocation / CAE retry feature behind the `AZURE_COSMOS_AAD_TOKEN_REVOCATION_ENABLED` environment variable (default OFF). When unset, the SDK preserves the exact pre-feature AAD behavior; setting the variable to `true` opts in to the revocation retry and `cp1` CAE capability.
