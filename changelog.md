...
- [PR-TBD](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/PR-TBD) Diagnostics: Fixes `InvalidOperationException: Collection was modified; enumeration operation may not execute.` raised from `TraceWriter` while serializing `ClientSideRequestStatisticsTraceDatum` during cross-region read hedging. The trace serializers and trace-summary aggregators now take a defensive snapshot of `ContactedReplicas`, `FailedReplicas`, and `RegionsContacted` before iterating, so diagnostics logging is safe when Direct-package store-reader paths mutate those collections in parallel.
