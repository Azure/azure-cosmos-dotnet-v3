...
- [5934](https://github.com/Azure/azure-cosmos-dotnet-v3/issues/5934) DistributedTransaction (preview): Adds conditional patch support to `DistributedWriteTransaction.PatchItem`. A new `DistributedTransactionPatchItemRequestOptions` (mirroring `TransactionalBatchPatchItemRequestOptions`) exposes a `FilterPredicate` SQL predicate that the server evaluates atomically before applying the patch, enabling scenarios such as state-machine transitions, bounded counter decrements, set-once writes, and per-tenant guards as part of a single atomic distributed transaction. The `PatchItem` `requestOptions` parameter type changes from `DistributedTransactionRequestOptions` to `DistributedTransactionPatchItemRequestOptions`; callers that pass `null`, omit the argument, or construct the options inline are unaffected, while callers holding an explicitly-typed `DistributedTransactionRequestOptions` variable should switch it to the new derived type.
