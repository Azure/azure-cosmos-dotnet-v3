...
- [5997](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/5997) Diagnostics: Adds an upper bound on the number of child traces retained under a single diagnostics node so a long-running operation that retries the same work many times (for example a cross-partition query whose background prefetch keeps retrying transport-generated `410`s against an unreachable region) can no longer grow its diagnostics to hundreds of megabytes. Once the limit is reached, additional child traces are omitted, the node reports how many were dropped via a `Truncated Child Trace Count` entry, and the top-level `Summary` is marked with `PartialResults: true`. Typical operations are unaffected; only pathological, effectively unbounded traces are truncated, in which case the `Summary` histogram counts are lower bounds (aggregate failure/region/hedging counts remain accurate).
