...
- [5988](https://github.com/Azure/azure-cosmos-dotnet-v3/pull/5988) Distributed Transactions (preview): Fixes a bodyless `429`/`3200` (RUBudgetExceeded) response on a distributed-transaction commit or read being surfaced to the caller without any retry. The status was classified coordinator-retriable but deferred to the throttling retry policy, which never re-sent the request on the distributed-transaction path. It is now retried by the same inner coordinator-retriable budget as `408` and `449`/`5352`, honoring the server's `x-ms-retry-after-ms` header. The shared coordinator-retriable budget is now also bounded by a cumulative-wait cap (60 seconds, matching the throttling retry policy's default) so a coordinator returning large retry-after values cannot stall a commit indefinitely.
